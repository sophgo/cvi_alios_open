...
{
	int ret = 0;
	partition_t handle = partition_open("weight");
	...
	if (handle < 0) {
		LOGE(TAG, "open weight partition failed");
		return -1;
	}
...
int app_cx_hw_init()
{
	...
	ret = cx_hardware_init(g_cx_boardconfig, &cx_mem_config);
	free(mem_config);
...
int app_cx_svr_init()
{
	return cx_service_init(g_cx_serviceconfig);
}
