            ...
            if(ai_load_feature(filePath,&g_AiFaceFeature,256) == 0) {
                for (uint32_t i = 0; i < pfaceMeta->size; i++) {
                    if(g_AiFaceFeature.ptr != NULL) {
                        if(ai_cal_cos_sim(&(pfaceMeta->info[i].feature),&g_AiFaceFeature,&score) == 0) {
                            if(score > CVIAI_WEIGHT) {
                                char str[128] = {0};
                                snprintf(str,sizeof(str),"%s score %0.2f",tmpPoint,score);
...
void *APP_Ai_Task(void *arg)
{
    ...
    cvi_tpu_init();
    aos_msleep(1000);
    s32Ret = CVI_AI_CreateHandle2(&g_AiHandle, -1, CVIAI_VPSS_GRP);
    ...
    while(g_RunStatus) {
        s32Ret = CVI_VPSS_GetChnFrame(CVIAI_VPSS_GRP, CVIAI_VPSS_CHN_AI,
                                    &stfdFrame, 2000);
        if(s32Ret != CVI_SUCCESS) {
            continue;
        }
        ...
        CVI_VPSS_ReleaseChnFrame(CVIAI_VPSS_GRP, CVIAI_VPSS_CHN_AI,&stfdFrame);
    }
EXIT:
    if(g_AiHandle) {
...
CVI_S32 APP_AiStart()
{
    ...
    s32Ret |= pthread_attr_setschedpolicy(&TaskAttr, SCHED_RR);
    s32Ret |= pthread_attr_setstacksize(&TaskAttr, 8192);
    s32Ret |= pthread_attr_setinheritsched(&TaskAttr, PTHREAD_EXPLICIT_SCHED);
...
void APP_AI_Register_Face(int argc,char **argv)
{
    ...
    if(faceMeta.size == 1) {
        ...
        //ai_feature_copy(&faceMeta.info[0].feature,&g_AiFaceFeature);
