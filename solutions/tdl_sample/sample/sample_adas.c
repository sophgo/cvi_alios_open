...
void* run_venc(void* args)
{
    ...
    while (bExit == false) {
        ...
        if (stLaneMeta.lane_state == 0) {
            ...
            // s32Ret = CVI_TDL_Service_ObjectWriteText(lane_info, txt_x, txt_y, &stFrame, 0, 255,
            // 0);
        } else {
            strcpy(lane_info, "LANE DEPARTURE WARNING !");
            // s32Ret = CVI_TDL_Service_ObjectWriteText(lane_info, txt_x, txt_y, &stFrame, 255, 0,
            // 0);
        }
        if (s32Ret != CVI_TDL_SUCCESS) {
        ...
        for (uint32_t i = 0; i < stLaneMeta.size; i++) {
            ...
            // s32Ret = CVI_TDL_Service_DrawPolygon(pstArgs->stServiceHandle, &stFrame, &pts,
            // brush_lane);
            if (s32Ret != CVI_TDL_SUCCESS) {
...
void* adas_main(void* arg)
{
    char** argv = (char**)arg;
    int argc    = 5;
    argv[0]     = "sample_tdl_adas";
    argv[1]     = "/mnt/sd/person_vehicle_cv181x.cvimodel";
    argv[2]     = "1";  // 0: only object; 1: object and lane
    argv[3]     = "/mnt/sd/lstr_hw.cvimodel";
    argv[4]     = "1";  // 0: lane_det model; 1: lstr model
    ...
    GOTO_IF_FAILED(
        CVI_TDL_OpenModel(stTDLHandle, CVI_TDL_SUPPORTED_MODEL_YOLOV8_DETECTION, argv[1]), s32Ret,
        setup_tdl_fail);
    ...
    if (argc == 5) {  // detect lane
        ...
        GOTO_IF_FAILED(CVI_TDL_OpenModel(stTDLHandle, lane_model_id, argv[3]), s32Ret,
                       setup_tdl_fail);
    }
...
void sample_adas(int argc, char** argv)
{
    pthread_attr_t attr;
    pthread_t thread;
    int ret;
    ...
    ret = pthread_create(&thread, &attr, adas_main, (void*)argv);
    if (ret != CVI_SUCCESS) {
...
ALIOS_CLI_CMD_REGISTER(sample_adas, sample_tdl_adas, cvi_tdl sample adas);
