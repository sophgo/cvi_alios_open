...
int process_image_file(cvitdl_handle_t tdl_handle, const char* imgf, cvtdl_face_t* p_obj)
{
    ...
    s32Ret =
        CVI_TDL_FaceDetection(tdl_handle, &stVideoFrame, CVI_TDL_SUPPORTED_MODEL_RETINAFACE, p_obj);
    if (s32Ret != CVI_SUCCESS) {
...
void* fd_main(void* arg)
{
    ...
    ret = CVI_TDL_OpenModel(tdl_handle, CVI_TDL_SUPPORTED_MODEL_RETINAFACE, fd_model);
    if (ret != CVI_SUCCESS) {
...
void sample_fd_image(int argc, char* argv[])
{
    pthread_attr_t attr;
    pthread_t thread;
    int ret;
...
ALIOS_CLI_CMD_REGISTER(sample_fd_image, sample_fd_image, cvi_tdl sample face detection with image);