...
CVI_S32 init_param(cvitdl_handle_t tdl_handle) {
  // setup preprocess
  InputPreParam preprocess_cfg = CVI_TDL_GetPreParam(tdl_handle, CVI_TDL_SUPPORTED_MODEL_YOLOV10_DETECTION);
  ...
  CVI_S32 ret = CVI_TDL_SetPreParam(tdl_handle, CVI_TDL_SUPPORTED_MODEL_YOLOV10_DETECTION, preprocess_cfg);
  if (ret != CVI_SUCCESS) {
  ...
  ret = CVI_TDL_SetDetectionAlgoParam(tdl_handle, CVI_TDL_SUPPORTED_MODEL_YOLOV10_DETECTION, yolov10_param);
  if (ret != CVI_SUCCESS) {
...
int process_image_file(cvitdl_handle_t tdl_handle, const char* imgf, cvtdl_face_t* p_obj)
{
    ...
    s32Ret = CVI_TDL_Detection(tdl_handle, &stVideoFrame, CVI_TDL_SUPPORTED_MODEL_YOLOV10_DETECTION, p_obj);
    if (s32Ret != CVI_SUCCESS) {
...
void* yolov10_main(void* arg)
{
    ...
    ret = CVI_TDL_OpenModel(tdl_handle, CVI_TDL_SUPPORTED_MODEL_YOLOV10_DETECTION, model);
    if (ret != CVI_SUCCESS) {
...
void sample_yolov10(int argc, char* argv[])
{
    pthread_attr_t attr;
    pthread_t thread;
    int ret;
    ...
    ret = pthread_create(&thread, &attr, yolov10_main, (void*)argv);
    if (ret != CVI_SUCCESS) {
...
ALIOS_CLI_CMD_REGISTER(sample_yolov10, sample_yolov10, cvi_tdl sample yolov10);