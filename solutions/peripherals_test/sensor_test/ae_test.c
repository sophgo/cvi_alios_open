...
static ISP_AE_STATISTICS_S stAeStat;

static AE_SENSOR_EXP_FUNC_S stExpFuncs[MAX_SENSOR_NUM];
static ISP_SNS_OBJ_S *pstSnsObj[MAX_SENSOR_NUM];

extern int self_scanf(const char *s_type,void *data);
extern int parse_str2int_space(const char *str,int *data,int num);
...
static void init_sensor_info(void)
{
	...
	for (int i = 0; i < MAX_SENSOR_NUM; i++) {
		pstSnsObj[i] = getSnsObj(snsr_type[i]);
...
{
#define AE_TEST_PARA_NUM    5

	CVI_S32 sID = 0, item = 0, para1 = 0, para2 = 0, para3 = 0;
	CVI_S32 s32Ret = CVI_SUCCESS;
    char str_buf[64] = {0,};
    int data[AE_TEST_PARA_NUM] = {0,};     //5 num

	sensor_ae_test_init();

	aos_cli_printf("\r\n1:AE_SetManualExposureTest(sID, 0:bypss 1:auto 2:manu, time, iso)\r\n");
	aos_cli_printf("2:AE_SetDebugMode(sID, item)\r\n");
	aos_cli_printf("3:AE_SetManualGainTest(sID, AG, DG, IG)\r\n");
	aos_cli_printf("4:AE_SetFpsTest(sID, fps)\r\n");
	aos_cli_printf("5:AE_SetLSC(sID, enable)\r\n");
	aos_cli_printf("6:AE_SetWDRManualRatio(sid, ratio), ratio: 4 - 256, 0: set SE max shutter time.\r\n");
	aos_cli_printf("7:AE_GainLinearTest(sID, time, startISO, endISO)\r\n");
	aos_cli_printf("8:AE_ShutterLinearTest(sID, fid 0: LE 1: SE, startExpTime, endExpTime)\r\n");
	aos_cli_printf("9:AE_GainTableLinearTest(sID, type: again 0 dgain 1, startIndex, endIndex)\r\n");
	aos_cli_printf("Item/sID/para1/para2/para3\n\r\n");

    self_scanf("%s",str_buf);
    s32Ret = parse_str2int_space(str_buf,data,AE_TEST_PARA_NUM);
    if (s32Ret != CVI_SUCCESS) {
        aos_cli_printf("parse str: %s fail\n",str_buf);
        return CVI_SUCCESS;
    }
    ...
    aos_cli_printf("item = %d ,sID = %d,para1 = %d,para2 = %d,para3 = %d\r\n"
                            ,item,sID,para1,para2,para3);
