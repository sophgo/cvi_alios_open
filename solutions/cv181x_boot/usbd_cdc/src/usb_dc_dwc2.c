...
static inline int dwc2_reset(void)
{
    ...
    /* Core Soft Reset */
    count = 0U;
   // printf("USB_OTG_GLB->GRSTCTL:%08x\n", USB_OTG_GLB->GRSTCTL);
    USB_OTG_GLB->GRSTCTL |= USB_OTG_GRSTCTL_CSRST;

    do {
        udelay(1);
        if (++count > 10000U) {
            printf("core soft reset failed!\n");
            return -1;
        }
...
static void dwc2_ep0_start_read_setup(uint8_t *psetup)
{
    ...
    //     printf("EER:ep0 start read, 0x%08x\n", USB_OTG_OUTEP(0U)->DOEPCTL);
    // }
...
int usbd_set_address(const uint8_t addr)
{
    ...
    //printf("after set address, DCFG:0x%08x\n", USB_OTG_DEV->DCFG);
    return 0;
}
    ...
    if (USB_EP_DIR_IS_OUT(ep)) {
        ...
        if (USB_OTG_OUTEP(ep_idx)->DOEPCTL & USB_OTG_DOEPCTL_EPENA) {
           ...
           // printf("goutnakeff success?\n");
            ...
            do {
                udelay(10);
                if (++count > 50000) {
                    printf("OUT ep disabled failed\n");
                    break;
                }
            } while ((USB_OTG_OUTEP(ep_idx)->DOEPINT & USB_OTG_DOEPINT_EPDISD) == 0);
          //  printf("OUT EP disabled success?\n");
        ...
        if (USB_OTG_INEP(ep_idx)->DIEPCTL & USB_OTG_DIEPCTL_EPENA) {
            ...
            do
            {
                ...
                if (++count > 200000U)
                {
                    printf("IN ep(%02x) close timeout\n", ep_idx);
                    return -1;
                }
            }
...
int usbd_ep_start_write(const uint8_t ep, const uint8_t *data, uint32_t data_len)
{
    ...
    if (ep_idx == 0) {
        ...
        if (pktcnt >= 1 && pktcnt <= 3) {
            ...
            USB_LOG_INFO("ep_mps(%d):%hu, data_len:%u, pktcnt:%u\n", ep_idx, g_dwc2_udc.in_ep[ep_idx].ep_mps, data_len, pktcnt);
           // printf("pktcnt is wrong! %u\n", pktcnt);
        }
    }
            ...
            while (ep_intr != 0U) {
                if ((ep_intr & 0x1U) != 0U) {
                    ...
                    if ((epint & USB_OTG_DIEPINT_EPDISD) == USB_OTG_DIEPINT_EPDISD) {
                        printf("ep(%d) disabled interrupt! \n", ep_idx);
