...
static void gc02m1_linear_1200p30_init(uint8_t ViPipe);

uint8_t gc02m1_i2c_addr = 0x37;
uint8_t gc02m1_i2c_dev = 1;
const uint32_t gc02m1_addr_byte = 1;
const uint32_t gc02m1_data_byte = 1;

int gc02m1_i2c_init(uint8_t ViPipe)
{
	uint8_t i2c_id = gc02m1_i2c_dev;
	int ret = sensor_i2c_init(i2c_id);

	return ret;
}

int gc02m1_i2c_exit(uint8_t ViPipe)
{
	uint8_t i2c_id = gc02m1_i2c_dev;
...
int gc02m1_read_register(uint8_t ViPipe, int addr)
{
	uint8_t i2c_id = gc02m1_i2c_dev;
	return sensor_i2c_read(i2c_id, gc02m1_i2c_addr, (uint32_t)addr, gc02m1_addr_byte, gc02m1_data_byte);
}

int gc02m1_write_register(uint8_t ViPipe, int addr, int data)
{
	uint8_t i2c_id = gc02m1_i2c_dev;

	return sensor_i2c_write(i2c_id, gc02m1_i2c_addr, (uint32_t)addr, gc02m1_addr_byte,
							(uint32_t)data, gc02m1_data_byte);
}

int gc02m1_probe(uint8_t i2c_addr, uint8_t i2c_dev)
{
	int nVal;
	int nVal2;
	uint8_t ViPipe = 0;

	gc02m1_i2c_addr = i2c_addr;
	gc02m1_i2c_dev = i2c_dev;

	if (gc02m1_i2c_init(ViPipe) != 0)
	{
		return -1;
	}
...
void gc02m1_init(uint8_t ViPipe)
{
	gc02m1_linear_1200p30_init(ViPipe);
}

void gc02m1_exit(uint8_t ViPipe)
{
