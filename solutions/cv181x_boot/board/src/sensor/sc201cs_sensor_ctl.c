...
static void sc201cs_linear_1200p30_init(uint8_t ViPipe);

uint8_t sc201cs_i2c_addr = 0x30;
uint8_t sc201cs_i2c_dev = 1;
const int sc201cs_addr_byte = 2;
const int sc201cs_data_byte = 1;

int sc201cs_i2c_init(uint8_t ViPipe)
{
	uint8_t i2c_id = (uint8_t)sc201cs_i2c_dev;

	return sensor_i2c_init(i2c_id);
}

int sc201cs_i2c_exit(uint8_t ViPipe)
{
	uint8_t i2c_id = (uint8_t)sc201cs_i2c_dev;
...
int sc201cs_read_register(uint8_t ViPipe, int addr)
{
	uint8_t i2c_id = (uint8_t)sc201cs_i2c_dev;

	return sensor_i2c_read(i2c_id, sc201cs_i2c_addr, (uint32_t)addr, sc201cs_addr_byte,
		sc201cs_data_byte);
}

int sc201cs_write_register(uint8_t ViPipe, int addr, int data)
{
	uint8_t i2c_id = (uint8_t)sc201cs_i2c_dev;

	return sensor_i2c_write(i2c_id, sc201cs_i2c_addr, (uint32_t)addr, sc201cs_addr_byte,
		(uint32_t)data, sc201cs_data_byte);
}

int sc201cs_probe(uint8_t i2c_addr, uint8_t i2c_dev)
{
	// int nVal;
	// int nVal2;
	uint8_t ViPipe = 0;
	...
	sc201cs_i2c_dev = i2c_dev;

	if (sc201cs_i2c_init(ViPipe) != 0)
		return -1;
...
void sc201cs_init(uint8_t ViPipe)
{
	sc201cs_linear_1200p30_init(ViPipe);
}

void sc201cs_exit(uint8_t ViPipe)
{
