...
int decompress_lz4(void *dst, size_t *dst_size, const void *src, size_t src_size)
{
	...
	if (LZ4F_isError(dctxStatus)) {
		printf("dctxStatus=%lu\n", dctxStatus);
		return -1;
	}

	ret = LZ4F_decompress(dctx, dst, dst_size, src, &src_size, NULL);
#if defined(CONFIG_DEBUG) && (CONFIG_DEBUG > 0)
	printf("dst_size=%lu src_size=%lu ret=%zd\n", *dst_size, src_size, ret);
#endif
	if (LZ4F_isError(dctxStatus)) {
		printf("LZ4F_decompress=%lu\n", ret);
		return -1;
	}
