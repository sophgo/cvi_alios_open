...
int CVI_MAPI_Media_Init(uint32_t img_w, uint32_t img_h, uint32_t blk_cnt) {
  ...
  u32BlkSize = COMMON_GetPicBufferSize(1920, 1080, PIXEL_FORMAT_RGB_888_PLANAR, DATA_BITWIDTH_8,
...
int CVI_MAPI_AllocateFrame(VIDEO_FRAME_INFO_S *frm, uint32_t width, uint32_t height,
                           PIXEL_FORMAT_E fmt) {
  VB_BLK blk;
  VB_CAL_CONFIG_S stVbCalConfig;
  COMMON_GetPicBufferConfig(width, height, fmt, DATA_BITWIDTH_8, COMPRESS_MODE_NONE, DEFAULT_ALIGN,
                            &stVbCalConfig);
  ...
  frm->stVFrame.u32Length[0] = ALIGN(stVbCalConfig.u32MainYSize, stVbCalConfig.u16AddrAlign);
...
static void get_frame_plane_num_and_mem_size(VIDEO_FRAME_INFO_S *frm, int *plane_num,
                                             size_t *mem_size) {
...
#define FILENAME_MAX_LEN (128)
  ...
  snprintf(filename, FILENAME_MAX_LEN, "%s_%dX%d.%s", name, frm->stVFrame.u32Width,
           frm->stVFrame.u32Height, extension);

  FILE *output;
  output = fopen(filename, "wb");
  assert(output && "file open failed");

  printf("Save %s, w*h(%d*%d)\n", filename, frm->stVFrame.u32Width, frm->stVFrame.u32Height);
  ...
  if (frm->stVFrame.enPixelFormat == PIXEL_FORMAT_YUV_PLANAR_420) {
    ...
    printf("Invalid frm pixel format");
  }
...
static CVI_S32 set_vpss_config(VPSS_GRP VpssGrp, VPSS_GRP_ATTR_S *stVpssGrpAttr,
                               PreprocessArg *arg) {
