...
class Hand_DetectionTestSuite : public CVI_TDLModelTestSuite {
 public:
  Hand_DetectionTestSuite() : CVI_TDLModelTestSuite("daily_reg_hand.json", "reg_daily_hand") {}
...
TEST_F(Hand_DetectionTestSuite, accuracy) {
  ...
  for (nlohmann::json::iterator iter = results.begin(); iter != results.end(); iter++) {
    ...
    ASSERT_EQ(CVI_TDL_Detection(m_tdl_handle, vframe, CVI_TDL_SUPPORTED_MODEL_HAND_DETECTION,
                                vehicle_meta),
              CVI_TDL_SUCCESS);
