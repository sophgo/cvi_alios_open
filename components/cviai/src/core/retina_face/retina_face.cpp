...
int RetinaFace::setupInputPreprocess(std::vector<InputPreprecessSetup> *data) {
  if (data->size() != 1) {
    LOGE(AISDK_TAG, "Retina face only has 1 input.\n");
...
void RetinaFace::outputParser(int image_width, int image_height, int frame_width, int frame_height,
                              cvai_face_t *meta) {
  std::map<std::string,float*> out_ptrs;
  for(int i = 0; i < mp_mi->out.num;i++){
    CVI_TENSOR *p_tensor = mp_mi->out.tensors+i;
    out_ptrs[p_tensor->name] = (float *)CVI_NN_TensorPtr(p_tensor);
  }
  ...
  for (uint32_t b = 0; b < (uint32_t)input_shape.dim[0]; b++) {
    ...
    for (size_t i = 0; i < m_feat_stride_fpn.size(); i++) {
      ...
      // size_t num_elements = width*height*blob_shape.dim[1];

      std::vector<anchor_box> anchors = m_anchors[bbox_str];
      if(num_anchor*count != anchors.size()){
        std::cout<<"w:"<<width<<",h:"<<height<<",shape1:"<<blob_shape.dim[1]<<std::endl;
        std::cout<<"anchors:"<<num_anchor<<",count:"<<count<<std::endl;
        // continue;
      }
    ...
    //std::cout<<"rawsize:"<<vec_bbox.size()<<",confthresh:"<<m_model_threshold<<std::endl;
