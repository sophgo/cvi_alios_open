...
static void wj_mbox_irq_handler(void *arg)
{
    ...
    for (uint32_t i = 0U; i < CHN_NUM; i++) {
        inter_state = wj_read_cpu_ch_interrupt_state(mbox_base, (uint32_t)current_cpu_id, i);

        if (inter_state & (uint32_t)((uint32_t)1U << 0)) {
    ...
    if (ret == CSI_OK) {
        ...
        for (uint32_t i = 0U; i < CHN_NUM; i++) {
            for (uint32_t j = 0U; j < INFO_NUM; j++) {
                mbox_base->MBOX_CPU[current_cpu_id].MBOX_CH[i].MBOX_INFO[j] = 0;
            }
...
int32_t csi_mbox_receive(csi_mbox_t *mbox, uint32_t channel_id, void *data, uint32_t size)
{
    ...
    if (channel_id < CHN_NUM) {
        ...
        wj_write_gen_cpu_ch_interrupt(mbox_base, target_cpu, target_channel, 0);   /// clear gen
