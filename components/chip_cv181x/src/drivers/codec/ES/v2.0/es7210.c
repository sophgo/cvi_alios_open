...
static int32_t reg_set_bit(es7210_dev_t *dev, uint8_t reg, uint8_t ops, uint8_t mask)
{
    uint8_t val = 0;
...
static int32_t reg_clear_bit(es7210_dev_t *dev, uint8_t reg, uint8_t ops, uint8_t mask)
{
    uint8_t val = 0;
    int ret = es7210_reg_read(dev, reg, &val);
...
static int32_t es7210_update_bits(es7210_dev_t *dev, unsigned char reg, unsigned char mask, unsigned char value)
{
...
static int32_t es7210_reset(es7210_dev_t *dev)
{
    int32_t ret = 0;
...
static int32_t es7210_mode(es7210_dev_t *dev, es7210_mode_t mode)
{
    int32_t ret = 0;
...
static int32_t es7210_protocol(es7210_dev_t *dev, es7210_protocol_t protocol)
{
    int32_t ret = 0;
    ...
    switch (protocol) {
        ...
        case ES7210_LSB_JUSTIFIED:
            ret |= es7210_update_bits(dev, ES7210_SDP_CFG1_REG11, 0x03, 0x01);
            break;

        case ES7210_I2S_TDM:
            ret |= es7210_update_bits(dev, ES7210_SDP_CFG1_REG11, 0x00, 0x00);
            ret |= es7210_update_bits(dev, ES7210_SDP_CFG2_REG12, 0x02, 0x02);
            ret |= es7210_update_bits(dev, ES7210_MODE_CFG_REG08, 0x04, 0x00);
            break;

        case ES7210_LEFT_JUSTIFIED_TDM:
            ret |= es7210_update_bits(dev, ES7210_SDP_CFG1_REG11, 0x01, 0x01);
            ret |= es7210_update_bits(dev, ES7210_SDP_CFG2_REG12, 0x02, 0x02);
            ret |= es7210_update_bits(dev, ES7210_MODE_CFG_REG08, 0x04, 0x00);
            break;

        case ES7210_DSPA_TDM:
            ret |= es7210_update_bits(dev, ES7210_SDP_CFG1_REG11, 0x03, 0x03);
            ret |= es7210_update_bits(dev, ES7210_SDP_CFG2_REG12, 0x01, 0x01);
            ret |= es7210_update_bits(dev, ES7210_MODE_CFG_REG08, 0x04, 0x00);
            break;

        case ES7210_DSPB_TDM:
            ret |= es7210_update_bits(dev, ES7210_SDP_CFG1_REG11, 0x13, 0x13);
            ret |= es7210_update_bits(dev, ES7210_SDP_CFG2_REG12, 0x01, 0x01);
            ret |= es7210_update_bits(dev, ES7210_MODE_CFG_REG08, 0x04, 0x00);
            break;
...
static int32_t es7210_data_len(es7210_dev_t *dev, es7210_data_len_t data_len)
{
    int32_t ret = 0;
...
static int32_t es7210_i2s_clk_divider(es7210_dev_t *dev, es7210_sclk_freq_t sclk_freq, es7210_mclk_freq_t mclk_freq)
{
    int32_t ret = 0;
    int32_t div = 0;

    div = mclk_freq / sclk_freq;
...
static int32_t es7210_1xfs_tdm_clk_divider(es7210_dev_t *dev, es7210_sclk_freq_t sclk_freq)
{
    int32_t ret = 0;

    switch (sclk_freq) {
        case ES7210_SCLK_64FS:
            ret = es7210_reg_write(dev, ES7210_MCLK_CTL_REG02, 0x41);
            ret = es7210_reg_write(dev, ES7210_DIGITAL_PDN_REG06, 0x00);
            break;

        case ES7210_SCLK_128FS:
            ret = es7210_reg_write(dev, ES7210_MCLK_CTL_REG02, 0x01);
            ret = es7210_reg_write(dev, ES7210_DIGITAL_PDN_REG06, 0x00);
            break;

        case ES7210_SCLK_256FS:
            ret = es7210_reg_write(dev, ES7210_MCLK_CTL_REG02, 0xc1);
            ret = es7210_reg_write(dev, ES7210_DIGITAL_PDN_REG06, 0x00);
            break;

        case ES7210_SCLK_1024FS:
            ret = es7210_reg_write(dev, ES7210_MCLK_CTL_REG02, 0x82);
            ret = es7210_reg_write(dev, ES7210_DIGITAL_PDN_REG06, 0x00);
            break;
...
int32_t es7210_init(es7210_dev_t *dev, es7210_config_t *es7210_config)
{
    ...
    es7210_mode(dev, es7210_config->mode);
    es7210_protocol(dev, es7210_config->protocol);

    es7210_data_len(dev, es7210_config->data_len);

    if (es7210_config->protocol == ES7210_I2S) {
        es7210_i2s_clk_divider(dev, es7210_config->sclk_freq, es7210_config->mclk_freq);
    } else if (es7210_config->protocol == ES7210_DSPB_TDM) {
        es7210_1xfs_tdm_clk_divider(dev, es7210_config->sclk_freq);
    }
