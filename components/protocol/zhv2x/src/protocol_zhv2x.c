...
void protocol_receive_input(unsigned char value)
{
    ...
    }else if((gp_queue_in > gp_queue_out) && ((gp_queue_in - gp_queue_out) >= sizeof(g_queue_buf))){
        //串口接收缓存已满
        // printf("[%s:%d]full\n", __FUNCTION__, __LINE__);
    }else {
        //队列不满
...
static void *protocol_service_req(void *args)
{
    ...
    while(1){
        ...
        while((rx_in < sizeof(g_potocol_rx_buf)) && get_queue_total_data() > 0 )
        {
            ...
            // printf("[%s:%d]g_potocol_rx_buf[%d]:%#x\n", __FUNCTION__, __LINE__, rx_in, g_potocol_rx_buf[rx_in]);
            rx_in ++;
        ...
        while ((rx_in - offset) >= PROTOCOL_HEAD)
        {
            ...
            rx_value_len = g_potocol_rx_buf[offset + LENGTH_HIGH] * 0x100 + g_potocol_rx_buf[offset + LENGTH_LOW];
            // printf("[%s:%d]rx_value_len:%#x\n", __FUNCTION__, __LINE__, rx_value_len);

            //数据接收完成
            if(get_check_sum((unsigned char *)g_potocol_rx_buf + offset + 2,rx_value_len + 3) != g_potocol_rx_buf[offset + rx_value_len + 5])
            {
                for(int i =0;i<rx_in+1;i++)
                aos_debug_printf("0x%x ",g_potocol_rx_buf[i]);

                DEBUG_PRINT("checksum = 0x%x cur = 0x%x\n",get_check_sum((unsigned char *)g_potocol_rx_buf + offset + 2,rx_value_len + 3), g_potocol_rx_buf[offset + rx_value_len + 5]);
                //校验出错
...
static void msg_reply_init_encryption(MR_STATUS_E result)
{
    ...
    unsigned char device_id[32] = {0xB9, 0x87, 0x85, 0xD6, 0xD6, 0xD1, 0xA6, 0x72, 0xB6, 0xA4, 0x21, 0xB7, 0xAD, 0x7E, 0x0F, 0x98, 0x83, 0xBA, 0x62, 0x5B, 0x52, 0x83, 0x50, 0x2C, 0xC7, 0x4A, 0xF9, 0xAE, 0x2C, 0x2F, 0x84, 0x03};
...
static void msg_reply_snap_image(MR_STATUS_E result)
{
    ...
    g_potocol_tx_buf[DATA_START + 1] = result;
    g_potocol_tx_buf[DATA_START + 2] = get_check_sum((unsigned char *)g_potocol_tx_buf + 2, 1 + 2 + 2);
...
static void msg_reply_get_saved_image(MR_STATUS_E result, s_msg_reply_get_saved_image_data *res_msg)
{
    ...
    g_potocol_tx_buf[DATA_START + 6] = get_check_sum((unsigned char *)g_potocol_tx_buf + 2, 1 + 2 + 6);
...
返回参数 : 无
*****************************************************************************/
static void msg_reply_verify(MEV_STATUS_E result, s_msg_reply_verify_data *ret_msg)
{
    g_potocol_tx_buf[HEAD_FIRST] = FRAME_FIRST;
    g_potocol_tx_buf[HEAD_SECOND] = FRAME_SECOND;
    g_potocol_tx_buf[MSG_ID] = MID_REPLY;
    unsigned short len = 38;
    if(MEV_SUCCESS == result){
        g_potocol_tx_buf[LENGTH_HIGH] =  len >> 8;
        g_potocol_tx_buf[LENGTH_LOW] = len & 0xff;
        ...
        g_potocol_tx_buf[DATA_START + 38] = get_check_sum((unsigned char *)g_potocol_tx_buf + 2, len + 3);
        protocol_send((unsigned char *)g_potocol_tx_buf, len + 6);
    }else{
        len = 2;
        g_potocol_tx_buf[LENGTH_HIGH] = 0x00;
        ...
        g_potocol_tx_buf[DATA_START] = MID_VERIFY;
        g_potocol_tx_buf[DATA_START + 1] = result;
        g_potocol_tx_buf[DATA_START + 2] = get_check_sum((unsigned char *)g_potocol_tx_buf + 2, len + 3);
        protocol_send((unsigned char *)g_potocol_tx_buf, len + 6);
    }
}
...
static void msg_reply_face_state(s_note_data_face *ret_msg)
{
    ...
    g_potocol_tx_buf[DATA_START + 17] = get_check_sum((unsigned char *)g_potocol_tx_buf + 2, 1 + 2 + 17);
...
返回参数 : 无
*****************************************************************************/
static void msg_reply_enroll(MEV_STATUS_E result, s_msg_reply_enroll_data *ret_msg)
{
    g_potocol_tx_buf[HEAD_FIRST] = 0xEF;
    g_potocol_tx_buf[HEAD_SECOND] = 0xAA;
    g_potocol_tx_buf[MSG_ID] = MID_REPLY;
    unsigned short len = 5;
    if(result == MEV_SUCCESS){
        g_potocol_tx_buf[LENGTH_HIGH] = len >> 8;
        g_potocol_tx_buf[LENGTH_LOW] = len & 0xff;
        ...
        g_potocol_tx_buf[DATA_START + 5] = get_check_sum((unsigned char *)g_potocol_tx_buf + 2, len + 3);
        protocol_send((unsigned char *)g_potocol_tx_buf, len + 6);
    }else{
        len = 2;
        g_potocol_tx_buf[LENGTH_HIGH] = len >> 8;
...
static void msg_reply_getuserinfo(MR_STATUS_E result, s_msg_reply_getuserinfo_data *res_msg)
{
    ...
    g_potocol_tx_buf[DATA_START + 24] = get_check_sum((unsigned char *)g_potocol_tx_buf + 2, 1 + 2 + 24);
...
返回参数 : 无
*****************************************************************************/
static void msg_reply_get_all_userid(MR_STATUS_E result, s_msg_reply_all_userid_data *res_msg)
{
    g_potocol_tx_buf[HEAD_FIRST] = FRAME_FIRST;
    g_potocol_tx_buf[HEAD_SECOND] = FRAME_SECOND;
    g_potocol_tx_buf[MSG_ID] = MID_REPLY;
    ...
    g_potocol_tx_buf[DATA_START + 1] = result;
    g_potocol_tx_buf[DATA_START + 2] = res_msg->user_counts;
    memcpy(&g_potocol_tx_buf[DATA_START + 3], res_msg->user_id, 100);
    g_potocol_tx_buf[DATA_START + 103] = get_check_sum((unsigned char *)g_potocol_tx_buf + 2, 1 + 2 + 103);
...
返回参数 : 无
*****************************************************************************/
static void msg_reply_enroll_itg(MEV_STATUS_E result, s_msg_reply_enroll_data *ret_msg)
{
    g_potocol_tx_buf[HEAD_FIRST] = FRAME_FIRST;
    g_potocol_tx_buf[HEAD_SECOND] = FRAME_SECOND;
    g_potocol_tx_buf[MSG_ID] = MID_REPLY;
...
static void msg_reply_get_version(MR_STATUS_E result, s_msg_reply_version_data *res_msg)
{
    ...
    g_potocol_tx_buf[DATA_START + 34] = get_check_sum((unsigned char *)g_potocol_tx_buf + 2, 1 + 2 + 34);
...
static void msg_reply_start_ota(void)
{
    ...
    memcpy(&g_potocol_tx_buf[DATA_START+2],reply_get_saved_image_data.image_size, sizeof(s_msg_reply_get_saved_image_data));
    // len += PROTOCOL_HEAD;
    DEBUG_PRINT("len = %d\n",len);
    check_sum = get_check_sum((unsigned char *)g_potocol_tx_buf + 2, len + 3);
    g_potocol_tx_buf[len + 5] = check_sum;
...
/*****************************************************************************
函数名称 : msg_reply_notify_wifi
功能描述 : 回复验证
输入参数 : 无
返回参数 : 无
*****************************************************************************/
static void msg_reply_notify_wifi(MR_STATUS_E result, s_msg_reply_open_data *res_msg)
{
    g_potocol_tx_buf[HEAD_FIRST] = FRAME_FIRST;
    g_potocol_tx_buf[HEAD_SECOND] = FRAME_SECOND;
    g_potocol_tx_buf[MSG_ID] = MID_REPLY;
    g_potocol_tx_buf[LENGTH_HIGH] = 0x00;
    g_potocol_tx_buf[LENGTH_LOW] = 0x05;
    g_potocol_tx_buf[DATA_START] = MID_NOTIFY_WIFI_INIT;
    g_potocol_tx_buf[DATA_START + 1] = result;
    g_potocol_tx_buf[DATA_START + 2] = res_msg->cmd_type;
    g_potocol_tx_buf[DATA_START + 3] = res_msg->ctrl_type;
    g_potocol_tx_buf[DATA_START + 4] = res_msg->mess_return;
    g_potocol_tx_buf[DATA_START + 5] = get_check_sum((unsigned char *)g_potocol_tx_buf + 2, 1 + 2 + 5);
    protocol_send((unsigned char *)g_potocol_tx_buf, 2 + 1 + 2 + 5 + 1);
}
...
static void msg_reply_getlibrary_version(s_msg_reply_library_version_data *res_msg)
{
    ...
    memcpy(&g_potocol_tx_buf[DATA_START + 2], res_msg->library_version_info, sizeof(s_msg_reply_library_version_data));
    g_potocol_tx_buf[DATA_START + 20] = get_check_sum((unsigned char *)g_potocol_tx_buf + 2, 1 + 2 + 20);
...
static void msg_reply_ready(void){
    ...
    g_potocol_tx_buf[DATA_START + 1] = get_check_sum((unsigned char *)g_potocol_tx_buf + 2, 1 + 2 + 1);
...
static void do_mid_enroll(void)
{
    ...
    memcpy(req_msg.user_name, g_potocol_rx_buf + DATA_START + 1, 4);
    req_msg.face_direction = g_potocol_rx_buf[DATA_START + 5];
    req_msg.timeout = g_potocol_rx_buf[DATA_START + 6];
...
{
    MR_STATUS_E result = MR_FAILED4_UNKNOWNREASON;
    s_msg_reply_all_userid_data res_msg;
    memset(&res_msg, 0, sizeof(s_msg_reply_all_userid_data));
    if(NULL != g_protocol_handles.get_all_userid){
        result = g_protocol_handles.get_all_userid(&res_msg);
    }else{
        result = MR_FAILED4_UNKNOWNREASON;
        printf("get_all_userid is NULL!\n");
    }
    printf("user_counts:%d\n", res_msg.user_counts);
    msg_reply_get_all_userid(result, &res_msg);
}
...
static void do_mid_reset(void)
{
    ...
    g_device_running_state.status = MS_READY;
...
static void do_mid_notify_wifi(void)
{
    MR_STATUS_E result = MR_SUCCESS;
    s_msg_wifi_init* preq_msg;
    s_msg_reply_open_data res_msg;
    uint16_t other_mess_len = ((g_potocol_rx_buf[3] << 8) | (g_potocol_rx_buf[4])) - 2;
    memset(&res_msg, 0, sizeof(s_msg_reply_open_data));
    preq_msg = (s_msg_wifi_init*)malloc(sizeof(s_msg_wifi_init) + other_mess_len);
    if(NULL != preq_msg){
        preq_msg->cmd_type = g_potocol_rx_buf[DATA_START];
        preq_msg->ctrl_type = g_potocol_rx_buf[DATA_START + 1];
            if(0 != other_mess_len){
        memcpy(preq_msg->other_mess, &g_potocol_rx_buf[DATA_START + 2], other_mess_len);
        }
        if(NULL != g_protocol_handles.notify_wifi){
            result = g_protocol_handles.notify_wifi(preq_msg, &res_msg);
        }else{
            result = MR_FAILED4_UNKNOWNREASON;
            printf("notify_wifi is NULL!\n");
        }
        free(preq_msg);
    }else{
        result = MR_FAILED4_UNKNOWNREASON;
        printf("do_mid_notify_wifi malloc filed");
    }
    msg_reply_notify_wifi(result, &res_msg);
}
...
void register_protocol_handles(protocol_handles_t* handles)
{
    ...
    g_protocol_handles.notify_wifi = handles->notify_wifi;
...
int32_t protocol_init(void)
{
    ...
    pthread_setname_np(pthreadId, "protocol_zhv2x_req");

    if (pthread_create(&pthreadId, &pthread_attr, protocol_service_res, NULL) != 0) {
        printf("%s: threadcreate err\n", __func__);
        // return;
    }
    pthread_setname_np(pthreadId, "protocol_zhv2x_res");
