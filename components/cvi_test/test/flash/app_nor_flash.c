...
static int nor_flash_read_unique_id(int argc, char **argv)
{
	int ret = 0;
	csi_spiflash_t spiflash_handle;
	uint32_t id[8] = {0};
	...
	csi_spiflash_spi_init(&spiflash_handle, spiflash_args.idx, NULL);
	ret = csi_spiflash_get_unique_id(&spiflash_handle, id, 8);

	return ret;
}
ALIOS_CLI_CMD_REGISTER(nor_flash_read_unique_id, read_unique_id, read unique id for nor flash);