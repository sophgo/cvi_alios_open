...
int imx675_read_register(VI_PIPE ViPipe, int addr)
{
	...
	return sensor_i2c_read(i2c_id, imx675_i2c_addr, (CVI_U32)addr, imx675_addr_byte, imx675_data_byte);
}
...
int imx675_write_register(VI_PIPE ViPipe, int addr, int data)
{
	...
	return sensor_i2c_write(i2c_id, imx675_i2c_addr, (CVI_U32)addr, imx675_addr_byte,
				(CVI_U32)data, imx675_data_byte);
}
...
void imx675_init(VI_PIPE ViPipe)
{
	...
	if (enWDRMode == WDR_MODE_2To1_LINE) {
		...
		if (u8ImgMode == IMX675_MODE_4M30) {
			imx675_linear_4M30_init(ViPipe);
		} else if (u8ImgMode == IMX675_MODE_5M30) {
...
static void imx675_linear_4M30_init(VI_PIPE ViPipe)
{
	...
	imx675_write_register(ViPipe, 0x3046, 0xA4); //PIX_VWIDTH[11:0]
	imx675_write_register(ViPipe, 0x3047, 0x05); //PIX_VWIDTH[11:0]
	...
	printf("ViPipe:%d,===IMX675 4M30fps 12bit LINE Init OK!===\n", ViPipe);
}
