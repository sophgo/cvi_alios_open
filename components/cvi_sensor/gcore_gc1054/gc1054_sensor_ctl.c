...
int gc1054_probe(VI_PIPE ViPipe)
{
	int nVal;
	int nVal2;
	usleep(50);
	if (gc1054_i2c_init(ViPipe) != CVI_SUCCESS)
		return CVI_FAILURE;

	nVal  = gc1054_read_register(ViPipe, GC1054_CHIP_ID_H);
	nVal2 = gc1054_read_register(ViPipe, GC1054_CHIP_ID_L);
	if (nVal < 0 || nVal2 < 0) {
		CVI_TRACE_SNS(CVI_DBG_ERR, "read sensor id error.\n");
		return nVal;
	}
	if ((((nVal & 0xFF) << 8) | (nVal2 & 0xFF)) != GC1054_CHIP_ID) {
