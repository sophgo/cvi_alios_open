...
{
	CVI_U8 i2c_id = (CVI_U8)g_aunCV2003_BusInfo[ViPipe].s8I2cDev;
...
int cv2003_read_register(VI_PIPE ViPipe, int addr)
{
	...
	return sensor_i2c_read(i2c_id, cv2003_i2c_addr, (CVI_U32)addr, cv2003_addr_byte, cv2003_data_byte);
}
...
int cv2003_write_register(VI_PIPE ViPipe, int addr, int data)
{
	...
	return sensor_i2c_write(i2c_id, cv2003_i2c_addr, (CVI_U32)addr, cv2003_addr_byte,
				(CVI_U32)data, cv2003_data_byte);
}
