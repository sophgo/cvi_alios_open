...
#include "cv2003_1L_slave_cmos_ex.h"
#include "cv2003_1L_slave_cmos_param.h"
#include "cvi_comm_cif.h"

#define DIV_0_TO_1(a)   ((0 == (a)) ? 1 : (a))
#define DIV_0_TO_1_FLOAT(a) ((((a) < 1E-10) && ((a) > -1E-10)) ? 1 : (a))
#define CV2003_1L_SLAVE_ID 2003
#define CV2003_1L_SLAVE_I2C_ADDR_1 0x35
#define CV2003_1L_SLAVE_I2C_ADDR_2 0x36
#define CV2003_1L_SLAVE_I2C_ADDR_IS_VALID(addr)      ((addr) == CV2003_1L_SLAVE_I2C_ADDR_1 || (addr) == CV2003_1L_SLAVE_I2C_ADDR_2)

/****************************************************************************
 * global variables                                                         *
 ***************************************************************************/

ISP_SNS_STATE_S *g_pastCV2003_1L_SLAVE[VI_MAX_PIPE_NUM] = {CVI_NULL};

#define CV2003_1L_SLAVE_SENSOR_GET_CTX(dev, pstCtx)   (pstCtx = g_pastCV2003_1L_SLAVE[dev])
#define CV2003_1L_SLAVE_SENSOR_SET_CTX(dev, pstCtx)   (g_pastCV2003_1L_SLAVE[dev] = pstCtx)
#define CV2003_1L_SLAVE_SENSOR_RESET_CTX(dev)         (g_pastCV2003_1L_SLAVE[dev] = CVI_NULL)

ISP_SNS_COMMBUS_U g_aunCV2003_1L_SLAVE_BusInfo[VI_MAX_PIPE_NUM] = {
	[0] = { .s8I2cDev = 3},
	[1 ... VI_MAX_PIPE_NUM - 1] = { .s8I2cDev = -1}
};

ISP_SNS_MIRRORFLIP_TYPE_E g_aeCV2003_1L_SLAVE_MirrorFip[VI_MAX_PIPE_NUM] = {0};

CVI_U16 g_au16CV2003_1L_SLAVE_GainMode[VI_MAX_PIPE_NUM] = {0};
...
/*****CV2003_1L_SLAVE Lines Range*****/
#define CV2003_1L_SLAVE_FULL_LINES_MAX  (0xfffff / 2)//liner mode: vts_reg value is double real_vts
// #define CV2003_1L_SLAVE_FULL_LINES_MAX_2TO1_WDR  (0xfffff / 4)//wdr mode: four

/*****CV2003_1L_SLAVE Register Address*****/
#define CV2003_1L_SLAVE_EXP1_ADDR0			0x304a //bit[19:16]
#define CV2003_1L_SLAVE_EXP1_ADDR1			0x3049
#define CV2003_1L_SLAVE_EXP1_ADDR2			0x3048
#define CV2003_1L_SLAVE_EXP2_ADDR0			0x3066
#define CV2003_1L_SLAVE_EXP2_ADDR1			0x3065
#define CV2003_1L_SLAVE_EXP2_ADDR2			0x3064

#define CV2003_1L_SLAVE_GAINENABLE_ADDR		0x3141 //bit[7:0]

#define CV2003_1L_SLAVE_AGAIN1_ADDR			0x3154 //bit[7:0]

#define CV2003_1L_SLAVE_DGAIN1_H_ADDR		0x314d //bit[15:8]
#define CV2003_1L_SLAVE_DGAIN1_L_ADDR		0x314c //bit[7:0]

#define CV2003_1L_SLAVE_VTS_ADDR0			0x3022 //bit[19:16]
#define CV2003_1L_SLAVE_VTS_ADDR1			0x3021
#define CV2003_1L_SLAVE_VTS_ADDR2			0x3020

#define CV2003_1L_SLAVE_FLIP_MIRROR_ADDR		0x3028

#define CV2003_1L_SLAVE_RES_IS_1080P(w, h)      ((w) == 1920 && (h) == 1080)

static CVI_S32 cmos_get_ae_default(VI_PIPE ViPipe, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
	const CV2003_1L_SLAVE_MODE_S *pstMode;

	CVI_U32 FpsMax;
	ISP_SNS_STATE_S *pstSnsState = CVI_NULL;

	CMOS_CHECK_POINTER(pstAeSnsDft);
	CV2003_1L_SLAVE_SENSOR_GET_CTX(ViPipe, pstSnsState);
	CMOS_CHECK_POINTER(pstSnsState);

	pstMode = &g_astCV2003_1L_SLAVE_mode[pstSnsState->u8ImgMode];
	FpsMax = g_astCV2003_1L_SLAVE_mode[pstSnsState->u8ImgMode].f32MaxFps;

	pstAeSnsDft->u32FullLinesStd = pstSnsState->u32FLStd;
	pstAeSnsDft->u32FlickerFreq = 50 * 256;
	pstAeSnsDft->u32FullLinesMax = CV2003_1L_SLAVE_FULL_LINES_MAX;
...
static CVI_S32 cmos_fps_set(VI_PIPE ViPipe, CVI_FLOAT f32Fps, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
	...
	u32Vts = g_astCV2003_1L_SLAVE_mode[pstSnsState->u8ImgMode].u32VtsDef;
	f32MaxFps = g_astCV2003_1L_SLAVE_mode[pstSnsState->u8ImgMode].f32MaxFps;
	f32MinFps = g_astCV2003_1L_SLAVE_mode[pstSnsState->u8ImgMode].f32MinFps;
	...
	if (pstSnsState->enWDRMode == WDR_MODE_NONE) {
		...
		u32VMAX = (u32VMAX > CV2003_1L_SLAVE_FULL_LINES_MAX) ?
				CV2003_1L_SLAVE_FULL_LINES_MAX : u32VMAX;
				    ...
				    CVI_U32 *au32IntTimeMax, CVI_U32 *au32IntTimeMin, CVI_U32 *pu32LFMaxIntTime)
{
	 ...
	 */
	u32IntTimeMaxTmp0 = ((pstSnsState->au32FL[1] - 1 - g_astCV2003_1L_SLAVE_mode[pstSnsState->u8ImgMode].u32IspResTime -
			pstSnsState->au32WDRIntTime[0]) * 0x40) / DIV_0_TO_1(au32Ratio[0]);
	u32IntTimeMaxTmp  = ((pstSnsState->au32FL[0] - 1 - g_astCV2003_1L_SLAVE_mode[pstSnsState->u8ImgMode].u32IspResTime)
...
static CVI_S32 cmos_get_wdr_size(VI_PIPE ViPipe, ISP_SNS_ISP_INFO_S *pstIspCfg)
{
	const CV2003_1L_SLAVE_MODE_S *pstMode = CVI_NULL;
	ISP_SNS_STATE_S *pstSnsState = CVI_NULL;
...
static CVI_S32 cmos_set_wdr_mode(VI_PIPE ViPipe, CVI_U8 u8Mode)
{
	...
	switch (u8Mode) {
	case WDR_MODE_NONE:
		if (pstSnsState->u8ImgMode == CV2003_1L_SLAVE_MODE_1920X1080P15_WDR)
			pstSnsState->u8ImgMode = CV2003_1L_SLAVE_MODE_1920X1080P30;

		pstSnsState->enWDRMode = WDR_MODE_NONE;
		pstSnsState->u32FLStd = g_astCV2003_1L_SLAVE_mode[pstSnsState->u8ImgMode].u32VtsDef;
...
static CVI_S32 cmos_get_sns_regs_info(VI_PIPE ViPipe, ISP_SNS_SYNC_INFO_S *pstSnsSyncInfo)
{
	...
	if ((pstSnsState->bSyncInit == CVI_FALSE) || (pstSnsRegsInfo->bConfig == CVI_FALSE)) {
		...
		pstCfg0->snsCfg.unComBus.s8I2cDev = g_aunCV2003_1L_SLAVE_BusInfo[ViPipe].s8I2cDev;
		...
		for (i = 0; i < pstCfg0->snsCfg.u32RegNum; i++) {
			...
			pstI2c_data[i].u8DevAddr = cv2003_1l_slave_i2c_addr;
			pstI2c_data[i].u32AddrByteNum = cv2003_1l_slave_addr_byte;
			pstI2c_data[i].u32DataByteNum = cv2003_1l_slave_data_byte;
		}

		switch (pstSnsState->enWDRMode) {
		default:
			pstI2c_data[LINEAR_EXP_0].u32RegAddr = CV2003_1L_SLAVE_EXP1_ADDR0;
			pstI2c_data[LINEAR_EXP_1].u32RegAddr = CV2003_1L_SLAVE_EXP1_ADDR1;
			pstI2c_data[LINEAR_EXP_2].u32RegAddr = CV2003_1L_SLAVE_EXP1_ADDR2;
			pstI2c_data[LINEAR_GAINENABLE].u32RegAddr = CV2003_1L_SLAVE_GAINENABLE_ADDR;
			pstI2c_data[LINEAR_AGAIN].u32RegAddr = CV2003_1L_SLAVE_AGAIN1_ADDR;
			pstI2c_data[LINEAR_DGAIN_H].u32RegAddr = CV2003_1L_SLAVE_DGAIN1_H_ADDR;
			pstI2c_data[LINEAR_DGAIN_L].u32RegAddr = CV2003_1L_SLAVE_DGAIN1_L_ADDR;
			pstI2c_data[LINEAR_VTS_0].u32RegAddr = CV2003_1L_SLAVE_VTS_ADDR0;
			pstI2c_data[LINEAR_VTS_1].u32RegAddr = CV2003_1L_SLAVE_VTS_ADDR1;
			pstI2c_data[LINEAR_VTS_2].u32RegAddr = CV2003_1L_SLAVE_VTS_ADDR2;
			pstI2c_data[LINEAR_FLIP_MIRROR].u32RegAddr = CV2003_1L_SLAVE_FLIP_MIRROR_ADDR;
			break;
		}
...
static CVI_S32 cmos_set_image_mode(VI_PIPE ViPipe, ISP_CMOS_SENSOR_IMAGE_MODE_S *pstSensorImageMode)
{
	...
	if (pstSensorImageMode->f32Fps <= 30) {
		if (pstSnsState->enWDRMode == WDR_MODE_NONE) {
			if (CV2003_1L_SLAVE_RES_IS_1080P(pstSensorImageMode->u16Width, pstSensorImageMode->u16Height))
				u8SensorImageMode = CV2003_1L_SLAVE_MODE_1920X1080P30;
			else {
...
static CVI_VOID sensor_mirror_flip(VI_PIPE ViPipe, ISP_SNS_MIRRORFLIP_TYPE_E eSnsMirrorFlip)
{
	...
	if (pstSnsState->bInit == CVI_TRUE && g_aeCV2003_1L_SLAVE_MirrorFip[ViPipe] != eSnsMirrorFlip) {
		...
		g_aeCV2003_1L_SLAVE_MirrorFip[ViPipe] = eSnsMirrorFlip;
...
static CVI_VOID sensor_global_init(VI_PIPE ViPipe)
{
	...
	pstSnsState->enWDRMode = WDR_MODE_NONE;
	pstSnsState->u32FLStd  = g_astCV2003_1L_SLAVE_mode[pstSnsState->u8ImgMode].u32VtsDef;
	pstSnsState->au32FL[0] = g_astCV2003_1L_SLAVE_mode[pstSnsState->u8ImgMode].u32VtsDef;
	pstSnsState->au32FL[1] = g_astCV2003_1L_SLAVE_mode[pstSnsState->u8ImgMode].u32VtsDef;
...
static CVI_S32 sensor_rx_attr(VI_PIPE ViPipe, SNS_COMBO_DEV_ATTR_S *pstRxAttr)
{
	...
	memcpy(pstRxAttr, &cv2003_1l_slave_rx_attr, sizeof(*pstRxAttr));

	pstRxAttr->img_size.width = g_astCV2003_1L_SLAVE_mode[pstSnsState->u8ImgMode].astImg[0].stSnsSize.u32Width;
	pstRxAttr->img_size.height = g_astCV2003_1L_SLAVE_mode[pstSnsState->u8ImgMode].astImg[0].stSnsSize.u32Height;
	if (pstSnsState->enWDRMode == WDR_MODE_NONE) {
...
{
	SNS_COMBO_DEV_ATTR_S *pstRxAttr = &cv2003_1l_slave_rx_attr;
	int i;
...
static CVI_S32 cmos_init_sensor_exp_function(ISP_SENSOR_EXP_FUNC_S *pstSensorExpFunc)
{
	...
	pstSensorExpFunc->pfn_cmos_sensor_init = cv2003_1l_slave_init;
	pstSensorExpFunc->pfn_cmos_sensor_exit = cv2003_1l_slave_exit;
...
{
	if (CV2003_1L_SLAVE_I2C_ADDR_IS_VALID(s32I2cAddr))
		cv2003_1l_slave_i2c_addr = s32I2cAddr;
}

static CVI_S32 cv2003_1l_slave_set_bus_info(VI_PIPE ViPipe, ISP_SNS_COMMBUS_U unSNSBusInfo)
{
	g_aunCV2003_1L_SLAVE_BusInfo[ViPipe].s8I2cDev = unSNSBusInfo.s8I2cDev;
...
static CVI_S32 sensor_ctx_init(VI_PIPE ViPipe)
{
	...
	CV2003_1L_SLAVE_SENSOR_GET_CTX(ViPipe, pastSnsStateCtx);
	...
	CV2003_1L_SLAVE_SENSOR_SET_CTX(ViPipe, pastSnsStateCtx);
...
static CVI_VOID sensor_ctx_exit(VI_PIPE ViPipe)
{
	...
	CV2003_1L_SLAVE_SENSOR_RESET_CTX(ViPipe);
	g_aeCV2003_1L_SLAVE_MirrorFip[ViPipe] = ISP_SNS_NORMAL;
}
...
static CVI_S32 sensor_register_callback(VI_PIPE ViPipe, ALG_LIB_S *pstAeLib, ALG_LIB_S *pstAwbLib)
{
	...
	stSnsAttrInfo.eSensorId = CV2003_1L_SLAVE_ID;
...
static CVI_S32 sensor_unregister_callback(VI_PIPE ViPipe, ALG_LIB_S *pstAeLib, ALG_LIB_S *pstAwbLib)
{
	...
	s32Ret = CVI_ISP_SensorUnRegCallBack(ViPipe, CV2003_1L_SLAVE_ID);
	if (s32Ret != CVI_SUCCESS) {
		CVI_TRACE_SNS(CVI_DBG_ERR, "sensor unregister callback function failed!\n");
		return s32Ret;
	}

	s32Ret = CVI_AE_SensorUnRegCallBack(ViPipe, pstAeLib, CV2003_1L_SLAVE_ID);
	if (s32Ret != CVI_SUCCESS) {
		CVI_TRACE_SNS(CVI_DBG_ERR, "sensor unregister callback function to ae lib failed!\n");
		return s32Ret;
	}

	s32Ret = CVI_AWB_SensorUnRegCallBack(ViPipe, pstAwbLib, CV2003_1L_SLAVE_ID);
	if (s32Ret != CVI_SUCCESS) {
...
static CVI_S32 sensor_set_init(VI_PIPE ViPipe, ISP_INIT_ATTR_S *pstInitAttr)
{
	...
	g_au16CV2003_1L_SLAVE_GainMode[ViPipe] = pstInitAttr->enGainMode;
...
{
	return cv2003_1l_slave_probe(ViPipe);
}

ISP_SNS_OBJ_S stSnsCV2003_1L_SLAVE_Obj = {
	.pfnRegisterCallback    = sensor_register_callback,
	.pfnUnRegisterCallback  = sensor_unregister_callback,
	.pfnStandby             = cv2003_1l_slave_standby,
	.pfnRestart             = cv2003_1l_slave_restart,
	.pfnWriteReg            = cv2003_1l_slave_write_register,
	.pfnReadReg             = cv2003_1l_slave_read_register,
	.pfnSetBusInfo          = cv2003_1l_slave_set_bus_info,
