...
extern char *AE_GetTimestampStr(void);
extern void AE_LogPrintf(const char *szFmt, ...);
...
// AE_LogPrintf("cmos_inttime_update time = %d \n", u32IntTime[0]);
...
static CVI_U32 gc1084_gain_table[21] = {
	1024,    
	/*1216,    1440,    1696,    2048,    */
	2432,    2864,    3392,
...
static CVI_S32 cmos_again_calc_table(VI_PIPE ViPipe, CVI_U32 *pu32AgainLin, CVI_U32 *pu32AgainDb)
{
	...
	total = sizeof(gc1084_gain_table) / sizeof(CVI_U32);
	setAgain = *pu32AgainLin;
	if (*pu32AgainLin >= gc1084_gain_table[total - 1]) {
		*pu32AgainLin = *pu32AgainDb = gc1084_gain_table[total - 1];
		return CVI_SUCCESS;
	}

	// for (i = 1; i < total; i++) {
	// 	if (*pu32AgainLin < gc1084_gain_table[i]) {
	// 		break;
	// 	}
	// }
	// i--;
	// find the pregain
	// pregain = *pu32AgainLin * 64 / gc1084_gain_table[i];
	...
	// } else {
	// 		*pu32AgainLin = pregain * gc1084_gain_table[i] / 64;
	// }
	...
	for (i = 1; i < total; i++) {
		if (*pu32AgainLin < gc1084_gain_table[i]) {
			*pu32AgainLin = gc1084_gain_table[i - 1];
			*pu32AgainDb = *pu32AgainLin;
			break;
		}
	}
...
AE_LogPrintf("%s", AE_GetTimestampStr());
AE_LogPrintf("test 2 ViPipe = %d cmos_again_calc_table setAgain = %d  again  = %d \n", ViPipe, setAgain, *pu32AgainLin);
	return CVI_SUCCESS;
}
...
// AE_LogPrintf("%s", AE_GetTimestampStr());	
// AE_LogPrintf("cmos_gains_update again = %u dgain =%u \n", pu32Again[0], pu32Dgain[0]);
	...
	if (pu32Again[0] < gc1084_gain_table[total - 1]) {
		for (i = 1; i < total; i++) {
			if (*pu32Again < gc1084_gain_table[i])
				break;
		}
		i--;
		// find the pregain
		u32Dgain = pu32Again[0] * 64 / gc1084_gain_table[i];
		u32Again = i;
	} else {
		// find the pregain
		u32Dgain = pu32Again[0] * 64 / gc1084_gain_table[total - 1];
		u32Again = total - 1;
	}
...
static CVI_S32 cmos_get_sns_regs_info(VI_PIPE ViPipe, ISP_SNS_SYNC_INFO_S *pstSnsSyncInfo)
{
	...
	if ((pstSnsState->bSyncInit == CVI_FALSE) || (pstSnsRegsInfo->bConfig == CVI_FALSE)) {
		...
		if (gainsUpdate) {
			...
			pstCfg0->snsCfg.astI2cData[LINEAR_DGAIN_H].bUpdate = CVI_FALSE;
			pstCfg0->snsCfg.astI2cData[LINEAR_DGAIN_L].bUpdate = CVI_FALSE;
		}
		if (expUpdate) {
