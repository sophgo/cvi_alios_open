...
#define SC202CS_CHIP_ID_ADDR_H	0x3107
#define SC202CS_CHIP_ID_ADDR_L	0x3108
#define SC202CS_CHIP_ID		0xeb52

#if CONFIG_SENSOR_DUAL_SWITCH
static void sc202cs_linear_1200p30_dual_switch_init(VI_PIPE ViPipe);
#else
static void sc202cs_linear_1200p30_init(VI_PIPE ViPipe);
#endif

CVI_U8 sc202cs_i2c_addr = 0x36;
const CVI_U32 sc202cs_addr_byte = 2;
const CVI_U32 sc202cs_data_byte = 1;

int sc202cs_i2c_init(VI_PIPE ViPipe)
{
	CVI_U8 i2c_id = (CVI_U8)g_aunSc202cs_BusInfo[ViPipe].s8I2cDev;
...
int sc202cs_read_register(VI_PIPE ViPipe, int addr)
{
	...
	return sensor_i2c_read(i2c_id, sc202cs_i2c_addr, (CVI_U32)addr, sc202cs_addr_byte,
		sc202cs_data_byte);
}
...
int sc202cs_write_register(VI_PIPE ViPipe, int addr, int data)
{
	...
	return sensor_i2c_write(i2c_id, sc202cs_i2c_addr, (CVI_U32)addr, sc202cs_addr_byte,
		(CVI_U32)data, sc202cs_data_byte);
}

void sc202cs_standby(VI_PIPE ViPipe)
{
	sc202cs_write_register(ViPipe, 0x3019, 0xff);
	sc202cs_write_register(ViPipe, 0x0100, 0x00);

	// printf("%s i2c_addr:%x, i2c_dev:%d\n", __func__, sc202cs_i2c_addr, g_aunSc202cs_BusInfo[ViPipe].s8I2cDev);
}

void sc202cs_restart(VI_PIPE ViPipe)
{
	sc202cs_write_register(ViPipe, 0x3019, 0xfe);
...
int sc202cs_probe(VI_PIPE ViPipe)
{
	...
	nVal  = sc202cs_read_register(ViPipe, SC202CS_CHIP_ID_ADDR_H);
	nVal2 = sc202cs_read_register(ViPipe, SC202CS_CHIP_ID_ADDR_L);
	if (nVal < 0 || nVal2 < 0) {
		CVI_TRACE_SNS(CVI_DBG_ERR, "read sensor id error.\n");
		return nVal;
	}

	if ((((nVal & 0xFF) << 8) | (nVal2 & 0xFF)) != SC202CS_CHIP_ID) {
...
{
#if CONFIG_SENSOR_DUAL_SWITCH
	sc202cs_linear_1200p30_dual_switch_init(ViPipe);
#else
	sc202cs_linear_1200p30_init(ViPipe);
#endif
...
static void sc202cs_linear_1200p30_dual_switch_init(VI_PIPE ViPipe)
{
	...
	sc202cs_write_register(ViPipe, 0x3019, 0xff); // mipi lane off
	sc202cs_write_register(ViPipe, 0x301f,0x01);
	...
	sc202cs_write_register(ViPipe, 0x0100, 0x00); // standby

	sc202cs_default_reg_init(ViPipe);

	sc202cs_restart(0);

	printf("ViPipe:%d, ===DUAL SC202CS 1200P 30fps 10bit LINEAR Init OK!===\n", ViPipe);
}
#else
...
static void sc202cs_linear_1200p30_init(VI_PIPE ViPipe)
{
	...
	printf("ViPipe:%d,===SC202CS 1200P 30fps 10bit LINEAR Init OK!===\n", ViPipe);
}
