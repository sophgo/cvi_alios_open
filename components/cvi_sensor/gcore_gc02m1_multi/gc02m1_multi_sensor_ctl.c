...
#define GC02M1_MULTI_MIRROR_NORMAL    1
#define GC02M1_MULTI_MIRROR_H         0
#define GC02M1_MULTI_MIRROR_V         0
#define GC02M1_MULTI_MIRROR_HV        0

#if GC02M1_MULTI_MIRROR_NORMAL
#define GC02M1_MULTI_MIRROR	        0x80
#elif GC02M1_MULTI_MIRROR_H
#define GC02M1_MULTI_MIRROR	        0x81
#elif GC02M1_MULTI_MIRROR_V
#define GC02M1_MULTI_MIRROR	        0x82
#elif GC02M1_MULTI_MIRROR_HV
#define GC02M1_MULTI_MIRROR	        0x83
#else
#define GC02M1_MULTI_MIRROR	        0x80
#endif

extern CVI_U16 g_au16Gc02m1_Multi_UseHwSync[VI_MAX_PIPE_NUM];
static void gc02m1_multi_linear_1200p30_init(VI_PIPE ViPipe);

CVI_U8 gc02m1_multi_i2c_addr = 0x37;
const CVI_U32 gc02m1_multi_addr_byte = 1;
const CVI_U32 gc02m1_multi_data_byte = 1;

int gc02m1_multi_i2c_init(VI_PIPE ViPipe)
{
	CVI_U8 i2c_id = (CVI_U8)g_aunGc02m1_Multi_BusInfo[ViPipe].s8I2cDev;

	return sensor_i2c_init(i2c_id);
}

int gc02m1_multi_i2c_exit(VI_PIPE ViPipe)
{
	CVI_U8 i2c_id = (CVI_U8)g_aunGc02m1_Multi_BusInfo[ViPipe].s8I2cDev;
...
int gc02m1_multi_read_register(VI_PIPE ViPipe, int addr)
{
	CVI_U8 i2c_id = (CVI_U8)g_aunGc02m1_Multi_BusInfo[ViPipe].s8I2cDev;

    return sensor_i2c_read(i2c_id, gc02m1_multi_i2c_addr, (CVI_U32)addr, gc02m1_multi_addr_byte, gc02m1_multi_data_byte);
}

int gc02m1_multi_write_register(VI_PIPE ViPipe, int addr, int data)
{
	CVI_U8 i2c_id = (CVI_U8)g_aunGc02m1_Multi_BusInfo[ViPipe].s8I2cDev;

    return sensor_i2c_write(i2c_id, gc02m1_multi_i2c_addr, (CVI_U32)addr, gc02m1_multi_addr_byte,
            (CVI_U32)data, gc02m1_multi_data_byte);
}

void gc02m1_multi_standby(VI_PIPE ViPipe)
{
	gc02m1_multi_write_register(ViPipe, 0x3e, 0x00);
}

void gc02m1_multi_restart(VI_PIPE ViPipe)
{
	gc02m1_multi_write_register(ViPipe, 0x3e, 0x90);
}

void gc02m1_multi_default_reg_init(VI_PIPE ViPipe)
{
	CVI_U32 i;

	for (i = 0; i < g_pastGc02m1_Multi[ViPipe]->astSyncInfo[0].snsCfg.u32RegNum; i++) {
		gc02m1_multi_write_register(ViPipe,
				g_pastGc02m1_Multi[ViPipe]->astSyncInfo[0].snsCfg.astI2cData[i].u32RegAddr,
				g_pastGc02m1_Multi[ViPipe]->astSyncInfo[0].snsCfg.astI2cData[i].u32Data);
	}
}

int gc02m1_multi_probe(VI_PIPE ViPipe)
{
#if CONFIG_SENSOR_QUICK_STARTUP
	if (gc02m1_multi_i2c_init(ViPipe) != CVI_SUCCESS)
		return CVI_FAILURE;
...
void gc02m1_multi_init(VI_PIPE ViPipe)
{
	CVI_U8		u8ImgMode = g_pastGc02m1_Multi[ViPipe]->u8ImgMode;
	WDR_MODE_E	enWDRMode =	g_pastGc02m1_Multi[ViPipe]->enWDRMode;
	gc02m1_multi_i2c_init(ViPipe);

	if (enWDRMode == WDR_MODE_2To1_LINE) {
		CVI_TRACE_SNS(CVI_DBG_ERR, "not surpport this WDR_MODE_E!\n");
	} else {
		if (u8ImgMode == GC02M1_MULTI_MODE_1600X1200P30)
	...
	g_pastGc02m1_Multi[ViPipe]->bInit = CVI_TRUE;
}

void gc02m1_multi_exit(VI_PIPE ViPipe)
{
...
static void gc02m1_multi_linear_1200p30_init(VI_PIPE ViPipe)
{
	...
	gc02m1_multi_write_register(ViPipe, 0x17, GC02M1_MULTI_MIRROR);
	...
	if (g_au16Gc02m1_Multi_UseHwSync[ViPipe]) {
		/*frame sync slave*/
		gc02m1_multi_write_register(ViPipe, 0xfe, 0x00);
		gc02m1_multi_write_register(ViPipe, 0x7f, 0x29);
		gc02m1_multi_write_register(ViPipe, 0x82, 0x08);
		gc02m1_multi_write_register(ViPipe, 0x83, 0x0f);
		gc02m1_multi_write_register(ViPipe, 0x88, 0x00);
		gc02m1_multi_write_register(ViPipe, 0x89, 0x04);
		gc02m1_multi_write_register(ViPipe, 0x8a, 0x00);
		gc02m1_multi_write_register(ViPipe, 0x8b, 0x12);
		gc02m1_multi_write_register(ViPipe, 0x85, 0x51);
	}

	gc02m1_multi_default_reg_init(ViPipe);
