## 第一部分： 基础信息
name: chip_cv184x                                   # <必选项> 包名称 （符合C语言变量命名规则），长度少于等于64字节
version: develop                                    # <必选项> 组件版本号
description: cv184x Chip CSI driver SDK.          # <必选项> 建议至少20字以上
...
source_file:
  ...
  - src/sys/trap_c.c
  - src/sys/weak.c
  - src/sys/dmesg.c ? <CONFIG_LOG_TO_DMESG>
  - src/sys/startup.S
  - src/sys/vectors.S
...
#  - src/drivers/timer/dw/v1.0/timer.c
  ...
  # - src/drivers/codec/cvitek/src/codec_adc.c
  # - src/drivers/codec/cvitek/src/codec_dac.c
