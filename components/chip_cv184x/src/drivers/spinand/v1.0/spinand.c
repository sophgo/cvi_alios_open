...
{
    spi_mem_op_t op = SPINAND_PAGE_READ_FROM_CACHE_X4_OP(nand_pos->offset,1,buf,size);
    csi_error_t  ret;
...
static csi_error_t spinand_write_to_cache_ext(csi_spinand_t *spinand, csi_nand_pos_t *nand_pos,void *buf, uint32_t size)
{
	...
	uint32_t    nbytes;
	uint32_t    adjust_len;
	/* use un-reset cache buf cmd */
    spi_mem_op_t op = SPINAND_PROG_LOAD_X4(0,0,spinand->xfer.xfer_buf,0);
	...
	/*write page-cache */
	memcpy(spinand->xfer.xfer_buf+nand_pos->offset,buf,size);

	while( nbytes ) {
		if(nbytes > spinand->max_op_size) {
			adjust_len = spinand->max_op_size;
...
uint32_t csi_spinand_frequence(csi_spinand_t *spinand, uint32_t hz)
{
    ...
    return csi_qspi_frequence((csi_qspi_t*)(&spinand->spi_qspi.qspi), hz);
}
