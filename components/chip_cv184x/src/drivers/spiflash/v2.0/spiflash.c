...
#define FLASH_CMD_QUAD_INPUT_PAGE_PROGRAM       0x32U
#define FLASH_CMD_READ_DATA          0x03U
#define FLASH_CMD_FAST_READ_QUAL     0x6bU
...
static int32_t spiflash_page_write(csi_spiflash_t *spiflash, uint32_t start_addr, void *data, uint32_t size)
{
    ...
    do {
        ...
        cmd       = FLASH_CMD_QUAD_INPUT_PAGE_PROGRAM;
        addr      = start_addr;
        ...
        cfg.data.buswidth=QSPI_CFG_BUS_QUAD;
        cfg.addr.buswidth=QSPI_CFG_BUS_SINGLE;
        cfg.dummy.nbytes=0;
        memset(spiflash->xfer.xfer_buf, 0xff, spiflash->xfer.xfer_buf_len);
        offset = start_addr % param->page_size;
        memcpy(( uint8_t * )spiflash->xfer.xfer_buf + offset, data, size);
        spiflash->set_cmd(spiflash, &cfg);
        ret = spiflash->spi_send(spiflash, cmd, addr - offset, addr_size, spiflash->xfer.xfer_buf, param->page_size);

        if(ret < 0){
            break;
        }
        else {
            count = ( uint32_t )(ret >= param->page_size)? size: ret;
        }
...
int32_t spiflash_page_read(csi_spiflash_t *spiflash, uint32_t offset, void *data, uint32_t size)
{
    CSI_PARAM_CHK(spiflash, CSI_ERROR);
    ...
    memset(spiflash->xfer.xfer_buf, 0xff, spiflash->xfer.xfer_buf_len);
    spiflash->set_cmd(spiflash, &cfg);
    offset = addr % param->page_size;
    ret = spiflash->spi_receive(spiflash, cmd, addr - offset, addr_size, spiflash->xfer.xfer_buf, param->page_size);
    memcpy(data, (uint8_t *)spiflash->xfer.xfer_buf + offset, size);
...
int32_t csi_spiflash_read(csi_spiflash_t *spiflash, uint32_t offset, void *data, uint32_t size)
{
    ...
    do {
        ret = spiflash_page_read(spiflash, current_addr, (void *)read_data, current_size);
