                               ...
                               csi_pwm_polarity_t polarity)
{
    ...
    const uint64_t count_unit = 250000000;  // 250M count per second
    ...
    if (CSI_OK == ret) {
        cvi_pwm_set_polarity_low_ch(reg_base, channel);

        duty_clk = (duty_clk * count_unit) / NSEC_COUNT;
        pr_debug("========period_clk: %llu, duty_clk:%llu=======\n", period_clk, duty_clk);
        cvi_pwm_set_high_period_ch(reg_base, channel, duty_clk);
        cvi_pwm_set_period_ch(reg_base, channel, period_clk);
        cvi_pwm_output_en_ch(reg_base, channel);
    }
...
csi_error_t csi_pwm_out_start(csi_pwm_t *pwm, uint32_t channel)
{
    ...
    cvi_pwm_start_en_ch(reg_base, channel);
...
void csi_pwm_out_stop(csi_pwm_t *pwm, uint32_t channel)
{
    ...
    cvi_pwm_start_dis_ch(reg_base, channel);
}
                                   ...
                                   uint32_t count)
{
    ...
    if (CSI_OK == ret) {
        ...
        cvi_pwm_input_en_ch(reg_base, channel);              ///< Enable time interrupt
    }
...
csi_error_t csi_pwm_capture_start(csi_pwm_t *pwm, uint32_t channel)
{
    ...
    cvi_cap_set_freqnum_ch(reg_base, channel, 1);
    cvi_cap_freq_en_ch(reg_base, channel);
...
void csi_pwm_capture_stop(csi_pwm_t *pwm, uint32_t channel)
{
    ...
    cvi_cap_freq_dis_ch(reg_base, channel);
