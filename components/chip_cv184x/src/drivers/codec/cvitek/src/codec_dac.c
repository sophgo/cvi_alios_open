...
static struct codec_dac g_dac;
static struct codec_dac *dac;

static int codec_dac_hw_params(struct codec_dac *dac, u32 chan_nr, u32 rate)
{
...
static void codec_dac_on(struct codec_dac *dac)
{
...
static void codec_dac_off(struct codec_dac *dac)
{
	u32 val = dac_read_reg(dac->dac_base, AUDIO_PHY_TXDAC_CTRL0);
...
static void codec_dac_shutdown(void)
{
	dac_reset();
	codec_dac_off(dac);
}

int codec_dac_ioctl(u32 cmd, struct cvi_vol_ctrl vol, u32 val)
{
...
static void dump_dac_reg(struct codec_dac *dac)
{
#ifdef __DEBUG__
...
int codec_dac_init(u32 rate, u32 chan_nr)
{
    ...
    dac->dac_base = (volatile u32 *)(0x0300A000);
	codec_dac_shutdown();
	codec_dac_hw_params(dac, chan_nr, rate);
	codec_dac_on(dac);
	dump_dac_reg(dac);
