 ...
 * CV184X CODEC driver on CVITEK CV184X
 *
...
codec_dac_init(config->sample_rate, 2);
...
csi_error_t csi_codec_output_mute(csi_codec_output_t *ch, bool enable)
{
    ...
    if(enable){
      ...
      //_/ es8156_set_software_mute(&es8156_dev, true);
		codec_dac_ioctl(ACODEC_SET_DACR_MUTE, vol, 1);
		codec_dac_ioctl(ACODEC_SET_DACL_MUTE, vol, 1);
    }
    return ret;
}
...
csi_error_t csi_codec_output_digital_gain(csi_codec_output_t *ch, uint32_t val)
{
	...
	codec_dac_ioctl(ACODEC_SET_OUTPUT_VOL, vol, val);
	return ret;
}
...
/**
  \brief       Set codec ouput lr channel analog gain.
  \param[in]   ch codec output channel handle.
  \param[in]   val gain val.
  \return      error code
*/
csi_error_t csi_codec_output_lr_analog_gain(csi_codec_output_t *ch, uint32_t stereo_id, uint32_t val)
{
    CSI_PARAM_CHK(ch, CSI_ERROR);
    csi_error_t ret = CSI_OK;
    struct cvi_vol_ctrl vol;
...
csi_error_t csi_codec_input_config(csi_codec_input_t *ch, csi_codec_input_config_t *config)
{
    ...
    if (i2s_codec_input.dev.reg_base == 0U) {
		...
		codec_adc_init(config->sample_rate);
	}
    return ret;
}
...
csi_error_t csi_codec_input_mute(csi_codec_input_t *ch, bool en)
{
    ...
    if(en){
      ...
      vol.vol_ctrl_mute = 1;
		...
		codec_adc_ioctl(ACODEC_SET_MICL_MUTE, vol, 1);
		codec_adc_ioctl(ACODEC_SET_MICR_MUTE, vol, 1);
    }
    return ret;
}
...
csi_error_t csi_codec_input_analog_gain(csi_codec_input_t *ch, uint32_t val)
{
	...
	codec_adc_ioctl(ACODEC_SET_ADCL_VOL, vol, val);
#if defined(CONFIG_CHIP_cv1842hp) || defined(CONFIG_CHIP_cv1843hp) || defined(CONFIG_CHIP_cv1841h)
	codec_adc_ioctl(ACODEC_SET_ADCR_VOL, vol, val);
#endif
    return ret;
}

/**
  \brief       Set codec input lr channel analog gain.
  \param[in]   ch codec input channel handle.
  \param[in]   val gain val.
  \return      error code
*/
csi_error_t csi_codec_input_lr_analog_gain(csi_codec_input_t *ch, uint32_t stereo_id, uint32_t val)
{
    CSI_PARAM_CHK(ch, CSI_ERROR);
    csi_error_t ret = CSI_OK;
    struct cvi_vol_ctrl vol;
