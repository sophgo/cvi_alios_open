...
static inline void _mdelay(uint32_t ms)
{
    uint64_t start_r = csi_coret_get_value();
    uint64_t cur_r;
    uint64_t cnt   = (soc_get_coretim_freq() / 1000U) * ms;

    while (1) {
        cur_r = csi_coret_get_value();

        if (start_r < cur_r) {
            if ((cur_r - start_r) >= cnt) {
                break;
            }
        } else {
            if (((MAX_UINT64 - start_r) + cur_r + 1) >= cnt) {
                break;
            }
        }
    }
...
static inline void _udelay(uint32_t us)udela
{
    uint64_t start_r = csi_coret_get_value();
    uint64_t cur_r;
    uint64_t cnt = (soc_get_coretim_freq() / 1000000U) * us;
