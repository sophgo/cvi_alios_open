...
/******************************************************************************
 * @file     cvitek.c
 * @brief    CSI Source File for cvitek ephy Driver
 * @version  V1.0
 * @date     21 July 2022
 ******************************************************************************/
...
static int cv184x_set_phy_loopback(eth_phy_handle_t handle, phy_loopback_mode_t mode)
{
	return 0;
}
#endif
...
/**
  \brief       Configure the cv184x before make it start up.
  \param[in]   handle  phy handle
  \return      error code
*/
/* CVITEK cv184x */
int32_t cv184x_config(eth_phy_handle_t handle)
{
...
#if defined(CVI_ETH_PHY_LOOPBACK)
	cv184x_set_phy_loopback(handle, LOOPBACK_PCS2MAC);
#endif
...
/**
  ...
  \return      error code
*/
static int32_t cv184x_parse_status(eth_phy_dev_t *dev)
{
...
/**
  ...
  \return      error code
*/
int32_t cv184x_start(eth_phy_handle_t handle)
{
    ETHPHY_NULL_PARAM_CHK(handle);
    ...
    return cv184x_parse_status(dev);
}

/**
  \brief       Halt the cv184x.
  \param[in]   handle  phy handle
  \return      error code
*/
int32_t cv184x_stop(eth_phy_handle_t handle)
{
    return 0;
}

/**
  \brief       Update the cv184x's link state.
  \param[in]   handle  phy handle
  \return      error code
*/
int32_t cv184x_update_link(eth_phy_handle_t handle)
{
    ETHPHY_NULL_PARAM_CHK(handle);
    eth_phy_dev_t *dev = (eth_phy_dev_t *)handle;
    return cv184x_parse_status(dev);;
...
/* Support for cv184x PHYs */
eth_phy_dev_t cv184x_device = {
	.name = "CVITEK,CV184X",
    .phy_id = 0x00435649,
    .mask = 0xffffffff,
    .features = PHY_BASIC_FEATURES,
    .config = &cv184x_config,
    .start = &cv184x_start,
    .stop = &cv184x_stop,
    //.loopback = &cv184x_loopback,
    //.update_link = &cv184x_update_link,
};
