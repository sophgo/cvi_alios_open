...
{
#define CLK_BASE_ADDRESS	0x03002000
#define CLK_AUD0_DIV		0xb4
#define CLK_AUD1_DIV		0xb8
#define CLK_AUD2_DIV		0xbc
#define CLK_AUD3_DIV		0xc0
#define CLK_AUDSRC_DIV		0xb0

#define PLL_G2_BASE_ADDRESS		0x03002800
#define PLL_G2_CTRL				0x00
#define PLL_A0PLL_CSR		    0x0C
#define PLL_SSC_SYN_CTRL		0x50
#define PLL_SSC_SYN_SET			0x54
#define PLL_SSC_SYN_SPAN		0x58
#define PLL_SSC_SYN_STEP		0x5C
    ...
    debug("%s \n", __func__);
	u32 aud_div = 0x00000001;//default 24.576Mhz
mmio_write_32((uintptr_t)(CLK_BASE_ADDRESS+CLK_AUD0_DIV), aud_div);
mmio_write_32((uintptr_t)(CLK_BASE_ADDRESS+CLK_AUD1_DIV), aud_div);
mmio_write_32((uintptr_t)(CLK_BASE_ADDRESS+CLK_AUD2_DIV), aud_div);
mmio_write_32((uintptr_t)(CLK_BASE_ADDRESS+CLK_AUD3_DIV), aud_div);
mmio_write_32((uintptr_t)(CLK_BASE_ADDRESS+CLK_AUDSRC_DIV), aud_div);
}
...
#if defined(ARCH_CV181X) || defined(ARCH_CV184X)
...
int i2s_init(struct i2stx_info *pi2s_tx)
{
 ...
 //       (struct i2s_sys_regs *)pi2s_tx->sys_base_address;
    ...
    if(pi2s_tx->id == I2S3)
	    i2s_set_sample_rate(i2s_reg, pi2s_tx->samplingrate, pi2s_tx->slot_no, "cv184x_dac"); /* sample rate must first prior to fmt */
    if(pi2s_tx->id == I2S0)
	    i2s_set_sample_rate(i2s_reg, pi2s_tx->samplingrate, pi2s_tx->slot_no, "cv184x_adc"); /* sample rate must first prior to fmt */
