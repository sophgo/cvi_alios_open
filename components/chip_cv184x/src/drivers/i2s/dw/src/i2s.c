 ...
 * CV184x I2s driver on CVITEK CV184x
 *
...
csi_error_t csi_i2s_format(csi_i2s_t *i2s, csi_i2s_format_t *format)
{
	...
	if (i2s_no == I2S0 || i2s_no == I2S3) {
		struct i2s_tdm_regs *i2s_reg = i2s_get_base(i2s_no);
...
static void i2s_proc_show(int32_t argc, char **argv)
{
	int idx = 0;
	if(argc == 2) {
		idx = (int)atoi(argv[1]);
...
ALIOS_CLI_CMD_REGISTER(i2s_proc_show, proc_i2s, i2s info);
