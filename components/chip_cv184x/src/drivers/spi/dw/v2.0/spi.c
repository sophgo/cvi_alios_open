...
#define IS_8BIT_FRAME_LEN(spi_base)   (dw_spi_get_data_frame_len(spi_base) <= 8U)
#define IS_16BIT_FRAME_LEN(spi_base)  (( dw_spi_get_data_frame_len(spi_base) > 8U ) && ( dw_spi_get_data_frame_len(spi_base) <= 16U ))

#define DW_SPI_HS_OFFSET               (2U)

#define DW_SPI_SET_REG_IDX_MASTER(spi) (spi->priv = (void *)0U)
#define DW_SPI_SET_REG_IDX_SLAVE(spi)  (spi->priv = (void *)1U)
#define DW_SPI_SET_REG_IDX_COMMON(spi) (spi->priv = (void *)1U)
#define DW_SPI_GET_REG_IDX(spi)        ((unsigned long)spi->priv)
#define IS_DW_SPI_IDX_MASTER(spi)      ((unsigned long)spi->priv == 0U)
#define IS_DW_SPI_IDX_SLAVE(spi)       ((unsigned long)spi->priv == 1U)
...
static dw_spi_regs_t *dw_get_reg_base(csi_spi_t *spi)
{
    dw_spi_regs_t *spi_base;
    unsigned long reg_base;
    uint32_t idx = 0U;

    idx = DW_SPI_GET_REG_IDX(spi);
    reg_base = HANDLE_REG_BASE(spi) + (idx * (uint32_t)0x100U);
    spi_base = (dw_spi_regs_t *)reg_base;
    return spi_base;
}

static uint16_t dw_spi_get_hs_num(csi_spi_t *spi, uint16_t *hs_num)
{
    uint16_t num = 0U;

    if (IS_DW_SPI_IDX_MASTER(spi)) {
        num = hs_num[( spi->dev.idx * DW_SPI_HS_OFFSET )];
    } else {
        num = hs_num[( spi->dev.idx * DW_SPI_HS_OFFSET ) + 1U];
    }
...
static csi_error_t wait_ready_until_timeout(csi_spi_t *spi, uint32_t timeout)
{
    ...
    dw_spi_regs_t *spi_base = dw_get_reg_base(spi);
...
csi_error_t csi_spi_init(csi_spi_t *spi, uint32_t idx)
{
    ...
    if (target_get(DEV_DW_SPI_TAG, idx, &spi->dev) != CSI_OK) {
        ...
        DW_SPI_SET_REG_IDX_MASTER(spi);
        spi_base = dw_get_reg_base(spi);
        dw_spi_disable_all_irq(spi_base);
        dw_spi_disable(spi_base);
        dw_spi_disable_slave_select_toggle(spi_base);

        DW_SPI_SET_REG_IDX_SLAVE(spi);
        spi_base = dw_get_reg_base(spi);
        dw_spi_disable_all_irq(spi_base);
        dw_spi_disable(spi_base);
        dw_spi_disable_slave_select_toggle(spi_base);
...
static csi_error_t dw_spi_send_dma(csi_spi_t *spi, const void *data, uint32_t size)
{
    ...
    do {
        ...
        config.handshake = dw_spi_get_hs_num(spi, spi_tx_hs_num);
...
static csi_error_t dw_spi_receive_dma(csi_spi_t *spi, void *data, uint32_t size)
{
    ...
    do {
        ...
        config.handshake = dw_spi_get_hs_num(spi, spi_rx_hs_num);;
        csi_dma_ch_config(dma_ch, &config);
