...
#define JTAG_TMS                     PA14
#define JTAG_TCLK                    PA13
#define WAKEUP_GPIO                  PA16

CODE_IN_RAM void soc_sleep(lpm_mode_e mode)
{
    wj_pmu_reg_t *pmu_base = (wj_pmu_reg_t *)WJ_PMU_BASE;
    cd_qspi_regs_t *qspi_base = (cd_qspi_regs_t *)CD_QSPI0_BASE;
    uint32_t backup_pd, backup_pu, backup_data;
    set_resume_func(pmu_base, (uint32_t *)&arch_resume_context);

    ///< Sync. with lc for close flash.
    while(wj_get_user_define_value(pmu_base,3) != 0xa5a5a5a5);
    wj_set_user_define_value(pmu_base,3,0);

    ///< Set all pins to gpio function except jtag point
    backup_pd = wj_get_gpio_pulldown();
    backup_pu = wj_get_gpio_pullup();
    backup_data = wj_get_gpio_func_select();

    wj_set_gpio_func_select((1 << JTAG_TMS) | (1 << JTAG_TCLK));
    wj_set_gpio_pulldown(~(1 << WAKEUP_GPIO));

    ///< Power down qspi
    cd_qspi_set_cmd_opcode(qspi_base, FALSH_CMD_POWER_DOWN);
    cd_qspi_enable_exe_cmd(qspi_base);

    wj_set_digitalio_to_gpio();
    wj_set_digitalio_pullup();
    ...
    wj_set_gpio_func_select(backup_data);
