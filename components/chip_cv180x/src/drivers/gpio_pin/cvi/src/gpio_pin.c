...
csi_error_t csi_gpio_pin_init(csi_gpio_pin_t *pin, pin_name_t pin_name)
{
    ...
    idx = cvi_pin_get_gpio_pinnum(pin_name);
    ...
    if (idx == 0xFFFFFFFFU) {
        ...
        if (!g_gpio_manage[idx].gpio.dev.reg_base) {
            csi_gpio_init(&g_gpio_manage[idx].gpio, g_gpio_manage[idx].gpio.dev.idx);
...
csi_error_t csi_gpio_pin_attach_callback(csi_gpio_pin_t *pin, void *callback, void *arg)
{
    ...
    unsigned long reg = (unsigned long)HANDLE_REG_BASE(pin->gpio);
    ...
    csi_gpio_attach_callback(pin->gpio, pin->callback, pin->arg);
