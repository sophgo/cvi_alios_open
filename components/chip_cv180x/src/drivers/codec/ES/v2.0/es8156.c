...
static inline int32_t es8156_set_sample_rate(es8156_dev_t *dev, es8156_i2s_sample_rate_t rate)
{
    ...
    switch (rate) {
        case ES8156_I2S_SAMPLE_RATE_48000:
            ...
            ret |= es8156_reg_write(dev, ES8156_MAIN_CLK_REG01, 0xa1U);
...
{
    if (en) {
        return es8156_update_bits(dev, ES8156_SDP_CFG1_REG11, 0x08, 0x01);
    }

    return es8156_update_bits(dev, ES8156_SDP_CFG1_REG11, 0x08, 0x00);
}
...
int32_t es8156_init(es8156_dev_t *dev, es8156_config_t *es8156_config)
{
    ...
    for (cnt = 0; cnt < sizeof(es8156_power_up_cfg) / sizeof(es8156_power_up_cfg[0]); cnt++) {
        ...
        es8156_reg_read(dev, es8156_power_up_cfg[cnt].reg_addr, &buf_config[cnt]);
        printf("init buf_config[%x] = %x\n",es8156_power_up_cfg[cnt].reg_addr,buf_config[cnt]);
    ...
    while(1)
    {
      es8156_read_all_register(dev,buf_config);
    }
    */
...
{
   // for (uint32_t cnt = 0; cnt < sizeof(es8156_power_up_cfg) / sizeof(es8156_power_up_cfg[0]); cnt++) {
    for (uint32_t cnt = 0; cnt < 38U; cnt ++) {
        es8156_reg_read(dev, cnt, &buf[cnt]);
    //    es8156_reg_read(dev, es8156_power_up_cfg[cnt].reg_addr, &buf_config[cnt]);
     //   printf("read_all buf_config[%x] = %x\n",es8156_power_up_cfg[cnt].reg_addr,buf_config[cnt]);
        printf("buf[%x] = %x\n",cnt,buf[cnt]);
    }
}
