...
void spi_hw_init(struct dw_spi *dws)
{
	...
	if (!dws->fifo_len) {
		...
		spi_debug("Detected FIFO size: %u bytes\n", dws->fifo_len);
	}
}
...
static inline uint32_t rx_max(struct dw_spi *dws)
{
	...
	spi_debug("data_in_fifo:%u temp: %u\n", data_in_fifo, temp);
	return temp;
}
...
void set_tran_mode(struct dw_spi *dws)
{
	...
	//uint32_t data_num = dw_read(dws, CVI_DW_SPI_CTRLR1);
	uint8_t tmode;
	...
	/* receive up to 64 KB of data(Data Frames) in a continuous transfer */
	//dw_writel(dws, CVI_DW_SPI_CTRLR1, data_num & 0xFFFF);
}
