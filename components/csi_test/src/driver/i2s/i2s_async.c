...
int test_i2s_async_send(test_i2s_args_t *args)
{
    csi_error_t status;
    uint32_t sent_num = 0;
    csi_i2s_t i2s;
    ...
    TEST_CASE_ASSERT_QUIT(status == CSI_OK, "i2s async send start failed");

    TEST_CASE_READY();
    sent_num = csi_i2s_send_async(&i2s, data_out, i2s_args.size);

    while (i2s_lock > 0);

    TEST_CASE_ASSERT(sent_num == i2s_args.size, "i2s async send data failed");
...
int test_i2s_async_receive(test_i2s_args_t *args)
{
    csi_error_t status;
    uint32_t recv_num = 0;
    csi_i2s_t i2s;
    ...
    dataset((char *)expect, i2s_args.size, 0x5Ay);
    TEST_CASE_READY();

    while (1) {
        recv_num += csi_i2s_receive_async(&i2s, (data_in + recv_num), (i2s_args.size - recv_num));

        if (recv_num == i2s_args.size) {
            TEST_CASE_TIPS("FINSHED");
            break;
        }
    }
