...
int rtc_time_assert(csi_rtc_time_t *ret_time, csi_rtc_time_t *expect)
{

    TEST_CASE_TIPS("Got RTC  time: %d-%d-%d %d:%d:%d", ret_time->tm_year + 1900, ret_time->tm_mon + 1, ret_time->tm_mday,
                   ret_time->tm_hour, ret_time->tm_min, ret_time->tm_sec);
    TEST_CASE_TIPS("Expected time: %d-%d-%d %d:%d",       expect->tm_year + 1900,    expect->tm_mon + 1, expect->tm_mday,
                   expect->tm_hour, expect->tm_min);
...
int calc_expect_rtc(csi_rtc_time_t *test_time, csi_rtc_time_t *expect, int sec_delta)
{
