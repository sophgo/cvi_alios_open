...
#define TEST_PRINT(format, ...) \
		do { \
			csi_test_atserver_send("+LOG:"); \
			csi_test_atserver_send(format, ##__VA_ARGS__); \
			}while(0)
...
#define TEST_PRINT_HEX(msg, data, data_len) \
	do{ \
		...
		for(int i=0;i<data_len; i++){       \
			csi_test_atserver_send("%02x", data[i]); \
		}                                   \
		csi_test_atserver_send("<<\r\n");              \
	}while(0)
...
#define TEST_CASE_TIPS(msg, ...) \
	do { \
		csi_test_atserver_send("\r\n+LOG:"); \
		csi_test_atserver_send("[INFO]-[%s]-[%s-%d]-[%s]-[", _mc_name, __FILENAME__, __LINE__, __func__); \
		csi_test_atserver_send(msg, ##__VA_ARGS__); \
		csi_test_atserver_send("]\r\n"); \
	}while(0)
#else
#define TEST_CASE_TIPS(msg, ...)
...
#define TEST_CASE_ASSERT(expression, msg, ...) \
	do { \
		if (!(expression)) { \
			_test_result = 1; \
			csi_test_atserver_send("\r\n+LOG:"); \
			csi_test_atserver_send("[ERROR]-[%s]-[%s-%d]-[%s]-[", _mc_name, __FILENAME__, __LINE__, __func__); \
			csi_test_atserver_send(msg, ##__VA_ARGS__); \
			csi_test_atserver_send("]\r\n"); \
		} \
	}while(0)
...
#define TEST_CASE_WARN(msg, ...) \
	do { \
		_test_result = 1; \
		csi_test_atserver_send("\r\n+LOG:"); \
		csi_test_atserver_send("[WARN]-[%s]-[%s-%d]-[%s]-[", _mc_name, __FILENAME__, __LINE__, __func__); \
		csi_test_atserver_send(msg, ##__VA_ARGS__); \
		csi_test_atserver_send("]\r\n"); \
	}while(0)
