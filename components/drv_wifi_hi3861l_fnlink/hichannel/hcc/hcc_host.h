...
#define HCC_TRIG_TX_SCHED_TIMEOUT           100
...
typedef struct {
    ...
    hi_u8  des_cnt;
    hi_u8  auc_resv[2];                 /* resv 2 $)A6TFk */
    oal_spin_lock_stru  st_hipri_lock;  /* $)A6AP4uc_hipriority_cntJ1R*<SKx */
...
typedef struct {
    hi_u8            hcc_transfer_thread;
    hi_u8            hcc_rx_thread;
    aos_sem_t        hcc_tx_wq;
    aos_sem_t        hcc_rx_wq;
    aos_sem_t              hcc_test_tx;
    hi_u32             channel_exception_flag;
    aos_timer_t         hcc_timer;
