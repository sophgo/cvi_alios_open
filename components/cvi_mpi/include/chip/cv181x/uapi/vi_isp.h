...
/* Enumeration for ISP scene configurations. */
enum ISP_SCENE_INFO {
    PRE_OFF_POST_OFF_SC,      // Pre-processing off, post-processing off (183x).
    PRE_OFF_POST_ON_SC,       // Pre-processing off, post-processing on (183x).
    FE_ON_BE_ON_POST_OFF_SC,  // Front-end on, back-end on, post-processing off (182x).
    FE_ON_BE_ON_POST_ON_SC,   // Front-end on, back-end on, post-processing on (182x).
    FE_ON_BE_OFF_POST_OFF_SC, // Front-end on, back-end off, post-processing off (182x).
    FE_ON_BE_OFF_POST_ON_SC,  // Front-end on, back-end off, post-processing on (182x).
    FE_OFF_BE_ON_POST_OFF_SC, // Front-end off, back-end on, post-processing off (182x).
    FE_OFF_BE_ON_POST_ON_SC,  // Front-end off, back-end on, post-processing on (182x).
};

/* Enumeration for ISP source types. */
enum cvi_isp_source {
...
/* Enumeration for IP information groups. */
enum IP_INFO_GRP {
...
#if !defined(__CV180X__)
    IP_INFO_ID_DPC1,             // ID for defective pixel correction 1.
#endif
...
struct ip_info {
    __u32 str_addr; // Start address of the IP.
    __u32 size;     // Total size of the IP registers.
};

/* Structure for memory block used in VIP processing. */
struct cvi_vip_memblock {
    __u8  raw_num;     // Number of raw data entries.
    __u64 phy_addr;    // Physical address of the memory block.
    void *vir_addr;    // Virtual address of the memory block.
#ifdef __arm__
    __u32 padding;     // Padding for alignment on ARM architecture.
#endif
    __u32 size;        // Size in bytes of the memory block.
};

/* Structure for raw block information in ISP processing. */
...
/* Structure for smooth raw parameters in ISP processing. */
...
/* Structure for ISP status memory management. */
...
/* Structure for ISP media bus frame format. */
...
/* Structure for ISP rectangle definition. */
struct cvi_isp_rect {
...
/* Structure for user picture configuration in ISP. */
...
/* Structure for sensor information in ISP. */
...
/* Structure for AWB status in ISP. */
...
#ifdef CONFIG_KERNEL_RHINO
/* Structure for ISP attributes in the RHINO kernel. */
struct cvi_isp_attr {
    aos_event_t        *event; // Event for synchronization.
