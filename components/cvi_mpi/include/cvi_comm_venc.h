...
#define CVI_VENC_DATA_FIFO_COUNT 11
...
typedef struct _venc_dbg_ {
	CVI_S32 startFn;    /* unused */
	CVI_S32 endFn;      /* unused */
	CVI_S32 dbgMask;    /* unused */
	CVI_S32 currMask;   /* debug level */
	CVI_CHAR dbgDir[CVI_VENC_STR_LEN];    /* unused */
	CVI_U32 noDataTimeout;    /* unused */
} venc_dbg;
...
#if ((CONFIG_CVI_LOG_TRACE_SUPPORT == 1) || defined(__linux__))
#ifdef ENABLE_SYSLOG
...
typedef enum {
    ...
    ///========//
	CVI_ERR_VENC_INIT = ENUM_ERR_VENC_VENC_INIT,            /* error in Initializing */
...
typedef union cviVENC_DATA_TYPE_U {
	...
	H265E_NALU_TYPE_E enH265EType; /* R; H265E NALU types*/
	PRORES_PACK_TYPE_E enPRORESType;  /* Apple ProRes pack type, unsupported now */
} VENC_DATA_TYPE_U;

/*the pack info of VENC*/
typedef struct _VENC_PACK_INFO_S {
	VENC_DATA_TYPE_U u32PackType; /* R; the pack type*/
	CVI_U32 u32PackOffset;        /* offset of pack */
	CVI_U32 u32PackLength;        /* length of pack */
} VENC_PACK_INFO_S;
...
typedef struct _VENC_STREAM_INFO_H264_S {
	...
	CVI_BOOL bPSkip;    /* whether current frame is pskip */
...
typedef struct _VENC_STREAM_S {
	...
	union {
		VENC_STREAM_INFO_H264_S stH264Info;		///< The information of h264 bitstream
		VENC_STREAM_INFO_JPEG_S stJpegInfo;		///< The information of JPEG bitstream
		VENC_STREAM_INFO_H265_S stH265Info;		///< The information of h265 bitstream
		VENC_STREAM_INFO_PRORES_S stProresInfo;	///< The information of Apple ProRes bitstream
	};

	union {
		VENC_STREAM_ADVANCE_INFO_H264_S
		stAdvanceH264Info;	///< the advance information of the h264e
		VENC_STREAM_ADVANCE_INFO_JPEG_S
		stAdvanceJpegInfo;	///< doesn't support jpeg advance info
		VENC_STREAM_ADVANCE_INFO_H265_S
		stAdvanceH265Info;	///< the advance information of the h265e
		VENC_STREAM_ADVANCE_INFO_PRORES_S
		stAdvanceProresInfo;///< doesn't support Apple ProRes advance info
	};
} VENC_STREAM_S;
...
typedef enum _VENC_PIC_RECEIVE_MODE_E {
	VENC_PIC_RECEIVE_SINGLE = 0,   /* current venc channel can receive pic from only ONE source */
	VENC_PIC_RECEIVE_MULTI,        /* current venc channel can receive pic from multiple source */
...
/* the frame rate of Apple ProRes */
...
/* the aspect ratio of Apple ProRes */
...
typedef struct _VENC_ATTR_S {
	...
	union {
		VENC_ATTR_H264_S stAttrH264e;	///< the attibute of h264 encoder
		VENC_ATTR_H265_S stAttrH265e;	///< the attibute of h265 encoder
		VENC_ATTR_JPEG_S stAttrJpege;	///< the attibute of JPEG encoder
		VENC_ATTR_PRORES_S stAttrProres;///< the attibute of Apple ProRes encoder
	};
} VENC_ATTR_S;
...
typedef struct _VENC_H264_VUI_S {
	VENC_VUI_ASPECT_RATIO_S stVuiAspectRatio;               /* the param of the vui aspect ratio*/
...
typedef struct _VENC_MOD_RC_S {
	CVI_U32 u32ClrStatAfterSetBr;    /* whether to clear RC statistics after set channel bitrate */
} VENC_MOD_RC_S;
...
typedef enum _VENC_FRAME_TYPE_E {
	VENC_FRAME_TYPE_NONE = 1,    /* don't specify frame type explicitly, actual frame type is decided by driver */
	VENC_FRAME_TYPE_IDR,         /* specify current frame to be encoded to IDR */
...
typedef struct _USER_FRAME_INFO_S {
	VIDEO_FRAME_INFO_S stUserFrame;    /* user frame info */
	USER_RC_INFO_S stUserRcInfo;       /* user rc info */
} USER_FRAME_INFO_S;
