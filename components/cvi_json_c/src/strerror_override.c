...
static int _cvi_json_c_strerror_enable = 0;
extern char *getenv(const char *name); // Avoid including stdlib.h

#define PREFIX "ERRNO="
static char errno_buf[128] = PREFIX;
char *cvi_json_c_strerror(int errno_in)
{
	int start_idx;
	char digbuf[20];
	int ii, jj;

	if (!_cvi_json_c_strerror_enable)
		_cvi_json_c_strerror_enable = (getenv("_JSON_C_STRERROR_ENABLE") == NULL) ? -1 : 1;
	if (_cvi_json_c_strerror_enable == -1)
