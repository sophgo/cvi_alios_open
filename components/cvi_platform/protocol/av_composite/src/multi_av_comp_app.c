...
static struct uvc_device_info uvc[AV_COMP_UVC_NUM] = {
    {
        .ep = 0x81,
...
static void vedio_streaming_send(struct uvc_device_info *uvc)
{
	...
	struct uvc_format_info_st uvc_format_info;
	//uint8_t *media_buffer = (uint8_t *)usb_iomalloc(DEFAULT_FRAME_SIZE);
...
//			printf("CVI_VPSS_GetChnFrame failed\n");
	...
	//memcpy(media_buffer, _acwindows1280x720, 300450);
	//buf_len = 300450;
	packets = usbd_video_payload_fill(media_buffer, buf_len, uvc->packet_buffer_uvc, &data_len);
	//usb_iofree(media_buffer);
	buf_len = 0;
	buf_len_stride = 0;
