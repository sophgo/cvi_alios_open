...
#include <ctype.h>
#include <stdio.h>
int tmp_strnicmp(const char *s1, const char *s2, size_t n) {
    size_t i = 0;
...
int getMemoryDataLines(const char *memoryData, char ** endpoint) {
    const char *start = memoryData;
    char *end;
...
static int getkeystring(INI_FILETYPE *fp, const TCHAR *Section, const TCHAR *Key,
                        ...
                        INI_FILEPOS *mark)
{
  TCHAR *sp = NULL, *ep =NULL;
  TCHAR *tmp_ep = NULL;
  int len, idx;
  enum quote_option quotes;
  TCHAR LocalBuffer[INI_BUFFERSIZE];
  unsigned int offset = 0;
  unsigned sum_len = strlen(*fp);
  assert(fp != NULL);
  ...
  if (len > 0 || idxSection >= 0) {
    ...
    idx = -1;
    do {
        memset(LocalBuffer, 0 ,INI_BUFFERSIZE);
        if(offset >= sum_len || getMemoryDataLines(*fp + offset, &tmp_ep))
            return 0;
        snprintf(LocalBuffer, tmp_ep - (*fp + offset) + 1, "%s", *fp + offset);
        offset = tmp_ep - *fp +1;
        //memcpy(LocalBuffer, , );
        //+= INI_BUFFERSIZE;
...
int ini_gets(const TCHAR *Section, const TCHAR *Key, const TCHAR *DefValue,
             TCHAR *Buffer, int BufferSize, TCHAR *Filename)
{
  INI_FILETYPE fp;
  int ok = 0;
 ...
 */
long ini_getl(const TCHAR *Section, const TCHAR *Key, long DefValue, TCHAR *Filename)
{
 ...
 * \return            the value located at Key
 */
INI_REAL ini_getf(const TCHAR *Section, const TCHAR *Key, INI_REAL DefValue, TCHAR *Filename)
{
  TCHAR LocalBuffer[64];
  int len = ini_gets(Section, Key, __T(""), LocalBuffer, sizearray(LocalBuffer), Filename);
 ...
 */
int ini_getbool(const TCHAR *Section, const TCHAR *Key, int DefValue, TCHAR *Filename)
{
 ...
 * \return            the number of characters copied into the supplied buffer
 */
int  ini_getsection(int idx, TCHAR *Buffer, int BufferSize, TCHAR *Filename)
{
  INI_FILETYPE fp;
  int ok = 0;
 ...
 * \return            the number of characters copied into the supplied buffer
 */
int  ini_getkey(const TCHAR *Section, int idx, TCHAR *Buffer, int BufferSize, TCHAR *Filename)
{
  INI_FILETYPE fp;
  int ok = 0;
 ...
 *                    (for success).
 */
int ini_browse(INI_CALLBACK Callback, void *UserData, TCHAR *Filename)
{
  TCHAR LocalBuffer[INI_BUFFERSIZE];
  int lenSec, lenKey;
  enum quote_option quotes;
...
static int close_rename(INI_FILETYPE *rfp, INI_FILETYPE *wfp, TCHAR *filename, TCHAR *buffer)
{
  (void)ini_close(rfp);
  (void)ini_close(wfp);
 ...
 */
int ini_puts(const TCHAR *Section, const TCHAR *Key, const TCHAR *Value, TCHAR *Filename)
{
  INI_FILETYPE rfp;
  INI_FILETYPE wfp;
  INI_FILEPOS mark;
 ...
 * \return            1 if successful, otherwise 0
 */
int ini_putl(const TCHAR *Section, const TCHAR *Key, long Value, TCHAR *Filename)
{
 ...
 * \return            1 if successful, otherwise 0
 */
int ini_putf(const TCHAR *Section, const TCHAR *Key, INI_REAL Value, TCHAR *Filename)
{
  TCHAR LocalBuffer[64];
