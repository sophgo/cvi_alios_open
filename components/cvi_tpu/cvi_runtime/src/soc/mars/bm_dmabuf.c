...
static DESC *traverse_start(uint8_t *cmdbuf)
{
  ...
  TPU_ASSERT(desc->hdr.magic == CMDBUF_HDR_MAGIC_MARS, NULL);
  return desc;
}
...
static DESC *traverse_next(DESC *desc, uint8_t *cmdbuf, size_t size)
{
  ...
  if ((uint8_t *)next_desc >= cmdbuf + size)
    return NULL;
  TPU_ASSERT(next_desc->hdr.magic == CMDBUF_HDR_MAGIC_MARS, NULL);
  return next_desc;
}
...
bmerr_t cvimars_dmabuf_relocate(uint8_t *dmabuf, uint64_t dmabuf_devaddr, uint32_t original_size, uint32_t pmubuf_size)
{
...
bmerr_t cvimars_dmabuf_convert(uint8_t *cmdbuf, size_t sz, uint8_t *dmabuf)
{
...
bmerr_t cvimars_dmabuf_size(uint8_t *cmdbuf, size_t sz, size_t *psize, size_t *pmu_size)
{
...
void cvimars_arraybase_set(uint8_t *dmabuf, uint32_t arraybase0L, uint32_t arraybase1L, uint32_t arraybase0H, uint32_t arraybase1H)
{
...
uint64_t cvimars_get_pmusize(uint8_t * dmabuf)
{
  ...
  pmu_size = ALIGN((tiu_cnt + tdma_cnt) * PER_DES_SIZE + PADDING_SIZE, 0x1000);
  //TPU_LOG_DEBUG("cvimars_get_pmusize pmusize= %" PRIu64 " \n", pmu_size);
  return pmu_size;
}
void cvimars_dmabuf_dump(uint8_t *dmabuf)
{
  TPU_ASSERT(dmabuf, NULL);
  dma_hdr_t *header = (dma_hdr_t *)dmabuf;
  ...
  for (u32 i = 0; i < header->cpu_desc_count; i++, desc++) {
    ...
    TPU_LOG_DEBUG("cvimars_dmabuf_dump num<bd:%d, tdma:%d>, offset<0x%08x, 0x%08x>\n", bd_num, tdma_num, bd_offset, tdma_offset);
  }
}
