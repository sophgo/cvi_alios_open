...
static void smtaudio_resume_list_insert(smtaudio_resume_list_node_t *smt_node)
{
    smtaudio_resume_list_node_t *first_node = dlist_first_entry(&smtaudio_resume_list_head, smtaudio_resume_list_node_t, node);

    /* 蓝牙播放被打断会有两次打断事件，本地和远程，如果本地打断已经在列表中，忽略远程打断 */
    if (first_node && smt_node) {
        if (first_node->id == smt_node->id) {
            return;
        }
    }
