...
extern void _check_and_dump_num(const char *filename, int num, char *buf, unsigned int sizebytes);
extern unsigned long long _get_current_pts(void);
extern unsigned long long _get_current_time(void);
CVI_BOOL CVI_Set_DRV_Pos(CVI_BOOL sel);
CVI_BOOL CVI_Get_DRV_Pos();
