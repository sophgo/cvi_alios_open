...
int CVI_RTSP_Create(CVI_RTSP_CTX **ctx, CVI_RTSP_CONFIG *config)
{
	...
	if (scheduler == nullptr) {
		SYSLOG(LOG_ERR, "fail to create CVI_TaskScheduler server");
	}

	UsageEnvironment *env = BasicUsageEnvironment::createNew(*scheduler);
	if (env == nullptr) {
		SYSLOG(LOG_ERR, "fail to create UsageEnvironment server");
	}
...
int CVI_RTSP_Start(CVI_RTSP_CTX *ctx)
{
	...
	if (0 > pthread_create(&(ctx->worker), &attr, rtspThread, (void *)ctx)) {
