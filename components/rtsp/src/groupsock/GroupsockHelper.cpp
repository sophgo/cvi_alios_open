...
#undef SO_REUSEPORT //for alios unsupport SO_REUSEPORT
...
int setupStreamSocket(UsageEnvironment& env,
  ...
  if (!initializeWinsockIfNecessary()) {
    socketErr(env, "Failed to initialize 'winsock': ");
    printf("initializeWinsockIfNecessary\n");
    return -1;
  }
  ...
  if (port.num() != 0 || ReceivingInterfaceAddr != INADDR_ANY) {
#endif
    MAKE_SOCKADDR_IN(name, htonl(INADDR_ANY), port.num());
    if (bind(newSocket, (struct sockaddr*)&name, sizeof name) != 0) {
      char tmpBuffer[100];
      sprintf(tmpBuffer, "bind() error (port number: %d): ",
	      ntohs(port.num()));
...
static unsigned increaseBufferTo(UsageEnvironment& env, int bufOptName,
  ...
  while (requestedSize > curSize) {
    socklen_t sizeSize = sizeof requestedSize;
    if (lwip_setsockopt(socket, SOL_SOCKET, bufOptName,
...
netAddressBits ourIPAddress(UsageEnvironment& env) {
  ...
  if (ourAddress == 0) {
	...
	break;
      ...
      if (netif && !ip_addr_isany(ip_2_ip4(&netif->ip_addr))) {
        fromAddr.sin_addr.s_addr = inet_addr(ipaddr_ntoa((ip4_addr_t *)ip_2_ip4(&netif->ip_addr)));
      }
#endif
