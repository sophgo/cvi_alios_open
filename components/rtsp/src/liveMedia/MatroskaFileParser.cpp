...
#ifdef RTSP_DEBUG
    fprintf(stderr, "\t=> seek time %f, file position %lu, block number within cluster %d\n", seekNPT, clusterOffsetInFile, blockNumWithinCluster);
#endif
...
#ifdef RTSP_DEBUG
	    fprintf(stderr, "Seeking to file position %lu (the previously-reported location of 'Cues')\n", fOurFile.fCuesOffset);
#endif
...
#ifdef RTSP_DEBUG
	    fprintf(stderr, "Optimization: Seeking to file position %lu (the previously-reported location of a 'Cluster')\n", fOurFile.fClusterOffset);
#endif
...
#ifdef RTSP_DEBUG
    fprintf(stderr, "MatroskaFileParser::parseStartOfFile(): Parsed id 0x%s (%s), size: %lu\n", id.hexString(), id.stringName(), size.val());
#endif
...
#ifdef RTSP_DEBUG
    fprintf(stderr, "MatroskaFileParser::lookForNextTrack(): Parsed id 0x%s (%s), size: %lu\n", id.hexString(), id.stringName(), size.val());
#endif
    switch (id.val()) {
...
#ifdef RTSP_DEBUG
	  fprintf(stderr, "\tSeek Position %lu (=> offset within the file: %lu (0x%lx))\n", seekPosition, offsetInFile, offsetInFile);
#endif
...
Boolean MatroskaFileParser::parseTrack() {
  ...
  while (fCurOffsetInFile < fLimitOffsetInFile) {
    ...
    fprintf(stderr, "MatroskaFileParser::parseTrack(): Parsed id 0x%s (%s), size: %lu\n", id.hexString(), id.stringName(), size.val());
#endif
    switch (id.val()) {
...
void MatroskaFileParser::lookForNextBlock() {
  ...
  while (fCurrentParseState == LOOKING_FOR_BLOCK) {
    while (!parseEBMLIdAndSize(id, size)) {}
#ifdef RTSP_DEBUG
    fprintf(stderr, "MatroskaFileParser::lookForNextBlock(): Parsed id 0x%s (%s), size: %lu\n", id.hexString(), id.stringName(), size.val());
#endif
    switch (id.val()) {
      case MATROSKA_ID_SEGMENT: { // 'Segment' header: enter this
	break;
	...
	break;
      ...
      // Attachments are parsed only if we're in RTSP_DEBUG mode (otherwise we just skip over them):
#ifdef RTSP_DEBUG
...
#if defined(RTSP_DEBUG) || defined(DEBUG_CUES)
...
void MatroskaFileParser::skipRemainingHeaderBytes(Boolean isContinuation) {
  ...
  while (fNumHeaderBytesToSkip > 0) {
    ...
    if (isContinuation || numBytesToSkipNow < fNumHeaderBytesToSkip) {
      fprintf(stderr, "\t\t(skipped %u bytes; %lu bytes remaining)\n",
