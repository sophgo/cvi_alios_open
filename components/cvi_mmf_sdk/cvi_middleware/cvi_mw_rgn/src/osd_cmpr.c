...
RGBA dec_literal(StreamBuffer *bs, OSD_FORMAT format)
{
	...
	if (format == OSD_ARGB8888) {
		...
		parse_stream(bs, (uint8_t *)&in_c.code, sizeof(in_c.code) * 8, false);
		CPY_C(in_c, color)
	} else if (format == OSD_ARGB4444) {
		ARGB4444 in_c;
		...
		if (md == Palette) {
			...
			if (p_ctrl->reg_osd_format == OSD_ARGB1555) {
				...
				if (run_len > 1) {
					...
					write_stream(bs, (uint8_t *)&_syntax, 19 + p_ctrl->reg_run_len_bd);
				} else {
					_syntax = (color.code << 1) | 1;
					write_stream(bs, (uint8_t *)&_syntax, 17);
				}
				...
				if (run_len > 1) {
					_syntax = ((uint64_t)code.color.code << 9) | ((run_len - 1) << 3) | 4;

					write_stream(bs, (uint8_t *)&_syntax, 35 + p_ctrl->reg_run_len_bd);
				} else {
					_syntax = ((uint64_t)code.color.code << 1) | 1;
...
void osd_cmpr_setup(OSDCmpr_Ctrl *p_ctrl, OSDCmpr_Cfg *p_cfg)
{
	...
	p_ctrl->bsbuf = (uint8_t *)malloc(p_ctrl->bs_buf_size * sizeof(uint8_t));
	if (p_ctrl->bsbuf == NULL) {
		printf("(%s %d) malloc(%d) failed!\n",
			__func__, __LINE__, (int)(p_ctrl->bs_buf_size * sizeof(uint8_t)));
	} else {
		memset(p_ctrl->bsbuf, 0x0, p_ctrl->bs_buf_size * sizeof(uint8_t));
	}
}
