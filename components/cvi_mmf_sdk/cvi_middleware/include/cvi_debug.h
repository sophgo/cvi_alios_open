...
#define CVI_TRACE(level, enModId, fmt, ...) 		   \
	do {                                                   \
		CVI_S32 LogLevel = (log_levels == NULL) ? CONFIG_CVI_LOG_TRACE_LEVEL : log_levels[enModId];      \
		if (level <= LogLevel)	       \
			aos_debug_printf("[%d][%s-%s] " fmt, csi_tick_get_ms(), CVI_GET_MOD_NAME(enModId), log_name[level],    \
			##__VA_ARGS__); 		  \
	} while (0)
#else
#define CVI_TRACE(level, enModId, fmt, ...) \
...
#define CVI_TRACE_TDE(level, fmt, ...)  \
		CVI_TRACE(level, CVI_ID_HDMI, "%s:%d:%s(): " fmt, __FILENAME__, __LINE__, __func__, ##__VA_ARGS__)
...
#define CVI_TRACE_MSG(level, fmt, ...)  \
		CVI_TRACE(level, CVI_ID_USER, "%s:%d:%s(): " fmt, __FILENAME__, __LINE__, __func__, ##__VA_ARGS__)
