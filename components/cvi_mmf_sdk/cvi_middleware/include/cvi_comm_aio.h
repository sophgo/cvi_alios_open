...
typedef struct _AI_TALKVQE_CONFIG_S {
    ...
    CVI_CHAR customize[AUD_MAX_VQE_CUSTOMIZE_NAME]; /* Customization name */
} AI_TALKVQE_CONFIG_S;
...
typedef struct _AUDIO_SAVE_FILE_INFO_S {
    ...
    CVI_CHAR    aFilePath[AUD_MAX_FILE_PATH_LEN]; /* File path where the audio is saved */
    CVI_CHAR    aFileName[AUD_MAX_FILE_NAME_LEN]; /* Name of the saved audio file */
