...
#define GC1054_I2C_ADDR_1 0x42
#define GC1054_I2C_ADDR_2 0x21
#define GC1054_I2C_ADDR_IS_VALID(addr)      ((addr) == GC1054_I2C_ADDR_1 || (addr) == GC1054_I2C_ADDR_2)
/****************************************************************************
 * global variables                                                            *
 ****************************************************************************/

ISP_SNS_STATE_S *g_pastGc1054[VI_MAX_PIPE_NUM] = {CVI_NULL};
SNS_COMBO_DEV_ATTR_S *g_pastGc1054ComboDevArray[VI_MAX_PIPE_NUM] = {CVI_NULL};
...
#define GC1054_SENSOR_SET_COMBO(dev, pstCtx)   (g_pastGc1054ComboDevArray[dev] = pstCtx)
#define GC1054_SENSOR_GET_COMBO(dev, pstCtx)   (pstCtx = g_pastGc1054ComboDevArray[dev])
...
ISP_SNS_COMMADDR_U g_aunGc1054_AddrInfo[VI_MAX_PIPE_NUM] = {
	[0] = { .s8I2cAddr = 0},
...
static CVI_S32 cmos_get_sns_regs_info(VI_PIPE ViPipe, ISP_SNS_SYNC_INFO_S *pstSnsSyncInfo)
{
	...
	if ((pstSnsState->bSyncInit == CVI_FALSE) || (pstSnsRegsInfo->bConfig == CVI_FALSE)) {
		...
		for (i = 0; i < pstCfg0->snsCfg.u32RegNum; i++) {
			...
			pstI2c_data[i].u8DevAddr = g_aunGc1054_AddrInfo[ViPipe].s8I2cAddr;
...
static CVI_S32 sensor_rx_attr(VI_PIPE ViPipe, SNS_COMBO_DEV_ATTR_S *pstRxAttr)
{
	...
	GC1054_SENSOR_GET_CTX(ViPipe, pstSnsState);
	GC1054_SENSOR_GET_COMBO(ViPipe, pstRxAttrSrc);
	CMOS_CHECK_POINTER(pstSnsState);
	...
	pstRxAttr->img_size.start_x = g_stGc1054_mode[pstSnsState->u8ImgMode].astImg[0].stWndRect.s32X;
	pstRxAttr->img_size.start_y = g_stGc1054_mode[pstSnsState->u8ImgMode].astImg[0].stWndRect.s32Y;
	pstRxAttr->img_size.active_w = g_stGc1054_mode[pstSnsState->u8ImgMode].astImg[0].stWndRect.u32Width;
	pstRxAttr->img_size.active_h = g_stGc1054_mode[pstSnsState->u8ImgMode].astImg[0].stWndRect.u32Height;
	pstRxAttr->img_size.width = g_stGc1054_mode[pstSnsState->u8ImgMode].astImg[0].stSnsSize.u32Width;
	pstRxAttr->img_size.height = g_stGc1054_mode[pstSnsState->u8ImgMode].astImg[0].stSnsSize.u32Height;
	pstRxAttr->img_size.max_width = g_stGc1054_mode[pstSnsState->u8ImgMode].astImg[0].stMaxSize.u32Width;
	pstRxAttr->img_size.max_height = g_stGc1054_mode[pstSnsState->u8ImgMode].astImg[0].stMaxSize.u32Height;
...
static CVI_S32 sensor_patch_rx_attr(VI_PIPE ViPipe, RX_INIT_ATTR_S *pstRxInitAttr)
{
	...
	if (!g_pastGc1054ComboDevArray[ViPipe]) {
		pstRxAttr = malloc(sizeof(SNS_COMBO_DEV_ATTR_S));
	} else {
		GC1054_SENSOR_GET_COMBO(ViPipe, pstRxAttr);
	}
	memcpy(pstRxAttr, &gc1054_rx_attr, sizeof(SNS_COMBO_DEV_ATTR_S));
	GC1054_SENSOR_SET_COMBO(ViPipe, pstRxAttr);
...
{
	if (g_pastGc1054ComboDevArray[ViPipe]) {
		free(g_pastGc1054ComboDevArray[ViPipe]);
		g_pastGc1054ComboDevArray[ViPipe] = CVI_NULL;
	}
...
{
	if (GC1054_I2C_ADDR_IS_VALID(s32I2cAddr))
		g_aunGc1054_AddrInfo[ViPipe].s8I2cAddr = s32I2cAddr;
	else {
		CVI_TRACE_SNS(CVI_DBG_ERR, "I2C addr input error ,please check [0x%x]\n", s32I2cAddr);
		g_aunGc1054_AddrInfo[ViPipe].s8I2cAddr = GC1054_I2C_ADDR_2;
	}
}
