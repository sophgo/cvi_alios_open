...
#define OS04C10_I2C_ADDR_1 0x36
#define OS04C10_I2C_ADDR_2 0x10
...
SNS_COMBO_DEV_ATTR_S *g_pastOs04c10ComboDevArray[VI_MAX_PIPE_NUM] = {CVI_NULL};
...
#define OS04C10_SENSOR_SET_COMBO(dev, pstCtx)   (g_pastOs04c10ComboDevArray[dev] = pstCtx)
#define OS04C10_SENSOR_GET_COMBO(dev, pstCtx)   (pstCtx = g_pastOs04c10ComboDevArray[dev])
...
ISP_SNS_COMMADDR_U g_aunOs04c10_AddrInfo[VI_MAX_PIPE_NUM] = {
	[0] = { .s8I2cAddr = 0},
...
static CVI_S32 cmos_set_wdr_mode(VI_PIPE ViPipe, CVI_U8 u8Mode)
{
	...
	case WDR_MODE_2To1_LINE:
		if (pstSnsState->u8ImgMode == OS04C10_MODE_2688X1520P30) {
			...
			CVI_TRACE_SNS(CVI_DBG_INFO, "WDR_MODE_2To1_LINE 1520p mode(60fps->30fps)\n");
...
static CVI_S32 cmos_get_sns_regs_info(VI_PIPE ViPipe, ISP_SNS_SYNC_INFO_S *pstSnsSyncInfo)
{
	...
	if ((pstSnsState->bSyncInit == CVI_FALSE) || (pstSnsRegsInfo->bConfig == CVI_FALSE)) {
		...
		for (i = 0; i < pstCfg0->snsCfg.u32RegNum; i++) {
			...
			pstI2c_data[i].u8DevAddr = g_aunOs04c10_AddrInfo[ViPipe].s8I2cAddr;
...
static CVI_S32 sensor_rx_attr(VI_PIPE ViPipe, SNS_COMBO_DEV_ATTR_S *pstRxAttr)
{
	...
	OS04C10_SENSOR_GET_CTX(ViPipe, pstSnsState);
	OS04C10_SENSOR_GET_COMBO(ViPipe, pstRxAttrSrc);
	...
	pstRxAttr->img_size.start_x = g_astOs04c10_mode[pstSnsState->u8ImgMode].astImg[0].stWndRect.s32X;
	pstRxAttr->img_size.start_y = g_astOs04c10_mode[pstSnsState->u8ImgMode].astImg[0].stWndRect.s32Y;
	pstRxAttr->img_size.active_w = g_astOs04c10_mode[pstSnsState->u8ImgMode].astImg[0].stWndRect.u32Width;
	pstRxAttr->img_size.active_h = g_astOs04c10_mode[pstSnsState->u8ImgMode].astImg[0].stWndRect.u32Height;
	...
	pstRxAttr->img_size.max_width = g_astOs04c10_mode[pstSnsState->u8ImgMode].astImg[0].stMaxSize.u32Width;
	pstRxAttr->img_size.max_height = g_astOs04c10_mode[pstSnsState->u8ImgMode].astImg[0].stMaxSize.u32Height;
...
static CVI_S32 sensor_patch_rx_attr(VI_PIPE ViPipe, RX_INIT_ATTR_S *pstRxInitAttr)
{
	...
	if (!g_pastOs04c10ComboDevArray[ViPipe]) {
		pstRxAttr = malloc(sizeof(SNS_COMBO_DEV_ATTR_S));
	} else {
		OS04C10_SENSOR_GET_COMBO(ViPipe, pstRxAttr);
	}
	memcpy(pstRxAttr, &os04c10_rx_attr, sizeof(SNS_COMBO_DEV_ATTR_S));
	OS04C10_SENSOR_SET_COMBO(ViPipe, pstRxAttr);
...
{
	if (g_pastOs04c10ComboDevArray[ViPipe]) {
		free(g_pastOs04c10ComboDevArray[ViPipe]);
		g_pastOs04c10ComboDevArray[ViPipe] = CVI_NULL;
	}
...
static CVI_VOID sensor_patch_i2c_addr(VI_PIPE ViPipe, CVI_S32 s32I2cAddr)
{
	if (OS04C10_I2C_ADDR_IS_VALID(s32I2cAddr))
		g_aunOs04c10_AddrInfo[ViPipe].s8I2cAddr = s32I2cAddr;
	else {
		CVI_TRACE_SNS(CVI_DBG_ERR, "I2C addr input error ,please check [0x%x]\n", s32I2cAddr);
		g_aunOs04c10_AddrInfo[ViPipe].s8I2cAddr = OS04C10_I2C_ADDR_2;
	}
}
