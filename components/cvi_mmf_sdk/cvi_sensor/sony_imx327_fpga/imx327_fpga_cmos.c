...
static CVI_S32 cmos_set_wdr_mode(VI_PIPE ViPipe, CVI_U8 u8Mode)
{
	...
	case WDR_MODE_2To1_LINE:
		...
		if (pstSnsState->u8ImgMode == IMX327_FPGA_MODE_1080P30_WDR) {
			...
			g_astImx327_fpga_State[ViPipe].u32BRL  = g_astImx327_fpga_mode[pstSnsState->u8ImgMode].u16BRL;
#if defined(CONFIG_DUAL_OS)
			printf("WDR_MODE_2To1_LINE 720p\n");
#else
			CVI_TRACE_SNS(CVI_DBG_INFO, "WDR_MODE_2To1_LINE 720p\n");
#endif
		}
		break;
	default:
...
static CVI_S32 sensor_rx_attr(VI_PIPE ViPipe, SNS_COMBO_DEV_ATTR_S *pstRxAttr)
{
	...
	pstRxAttr->img_size.start_x = g_astImx327_fpga_mode[pstSnsState->u8ImgMode].astImg[0].stWndRect.s32X;
	pstRxAttr->img_size.start_y = g_astImx327_fpga_mode[pstSnsState->u8ImgMode].astImg[0].stWndRect.s32Y;
	pstRxAttr->img_size.active_w = g_astImx327_fpga_mode[pstSnsState->u8ImgMode].astImg[0].stWndRect.u32Width;
	pstRxAttr->img_size.active_h = g_astImx327_fpga_mode[pstSnsState->u8ImgMode].astImg[0].stWndRect.u32Height;
	...
	pstRxAttr->img_size.max_width = g_astImx327_fpga_mode[pstSnsState->u8ImgMode].astImg[0].stMaxSize.u32Width;
	pstRxAttr->img_size.max_height = g_astImx327_fpga_mode[pstSnsState->u8ImgMode].astImg[0].stMaxSize.u32Height;

	if (pstSnsState->enWDRMode != WDR_MODE_NONE) {
		CVI_TRACE_SNS(CVI_DBG_INFO, "enWDRMode=%d\n", pstSnsState->enWDRMode);
