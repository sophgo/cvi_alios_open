...
#define SC501AI_2L_I2C_ADDR 0x30
#define SC501AI_2L_I2C_ADDR_IS_VALID(addr)	((addr) == SC501AI_2L_I2C_ADDR)
/****************************************************************************
 * global variables                                                            *
 ****************************************************************************/

ISP_SNS_STATE_S *g_pastSC501AI_2L[VI_MAX_PIPE_NUM] = {CVI_NULL};
SNS_COMBO_DEV_ATTR_S *g_pastSC501AI_2LComboDevArray[VI_MAX_PIPE_NUM] = {CVI_NULL};
...
#define SC501AI_2L_SENSOR_GET_COMBO(dev, pstCtx)   (pstCtx = g_pastSC501AI_2LComboDevArray[dev])
#define SC501AI_2L_SENSOR_SET_COMBO(dev, pstCtx)   (g_pastSC501AI_2LComboDevArray[dev] = pstCtx)
...
ISP_SNS_COMMADDR_U g_aunSC501AI_2L_AddrInfo[VI_MAX_PIPE_NUM] = {
	[0] = { .s8I2cAddr = 0},
...
static CVI_S32 cmos_get_sns_regs_info(VI_PIPE ViPipe, ISP_SNS_SYNC_INFO_S *pstSnsSyncInfo)
{
	...
	if ((pstSnsState->bSyncInit == CVI_FALSE) || (pstSnsRegsInfo->bConfig == CVI_FALSE)) {
		...
		for (i = 0; i < pstCfg0->snsCfg.u32RegNum; i++) {
			...
			pstI2c_data[i].u8DevAddr = g_aunSC501AI_2L_AddrInfo[ViPipe].s8I2cAddr;
...
static CVI_S32 sensor_rx_attr(VI_PIPE ViPipe, SNS_COMBO_DEV_ATTR_S *pstRxAttr)
{
	...
	SC501AI_2L_SENSOR_GET_CTX(ViPipe, pstSnsState);
	SC501AI_2L_SENSOR_GET_COMBO(ViPipe, pstRxAttrSrc);
	CMOS_CHECK_POINTER(pstSnsState);
	...
	pstRxAttr->img_size.start_x = g_astSC501AI_2L_mode[pstSnsState->u8ImgMode].astImg[0].stWndRect.s32X;
	pstRxAttr->img_size.start_y = g_astSC501AI_2L_mode[pstSnsState->u8ImgMode].astImg[0].stWndRect.s32Y;
	pstRxAttr->img_size.active_w = g_astSC501AI_2L_mode[pstSnsState->u8ImgMode].astImg[0].stWndRect.u32Width;
	pstRxAttr->img_size.active_h = g_astSC501AI_2L_mode[pstSnsState->u8ImgMode].astImg[0].stWndRect.u32Height;
	...
	pstRxAttr->img_size.max_width = g_astSC501AI_2L_mode[pstSnsState->u8ImgMode].astImg[0].stMaxSize.u32Width;
	pstRxAttr->img_size.max_height = g_astSC501AI_2L_mode[pstSnsState->u8ImgMode].astImg[0].stMaxSize.u32Height;
...
static CVI_S32 sensor_patch_rx_attr(VI_PIPE ViPipe, RX_INIT_ATTR_S *pstRxInitAttr)
{
	...
	if (!g_pastSC501AI_2LComboDevArray[ViPipe]) {
		pstRxAttr = malloc(sizeof(SNS_COMBO_DEV_ATTR_S));
	} else {
		SC501AI_2L_SENSOR_GET_COMBO(ViPipe, pstRxAttr);
	}
	memcpy(pstRxAttr, &sc501ai_2l_rx_attr, sizeof(SNS_COMBO_DEV_ATTR_S));
	SC501AI_2L_SENSOR_SET_COMBO(ViPipe, pstRxAttr);
...
{
	if (g_pastSC501AI_2LComboDevArray[ViPipe]) {
		free(g_pastSC501AI_2LComboDevArray[ViPipe]);
		g_pastSC501AI_2LComboDevArray[ViPipe] = CVI_NULL;
	}
...
{
	if (SC501AI_2L_I2C_ADDR_IS_VALID(s32I2cAddr))
		g_aunSC501AI_2L_AddrInfo[ViPipe].s8I2cAddr = s32I2cAddr;
	else {
		CVI_TRACE_SNS(CVI_DBG_ERR, "I2C addr input error ,please check [0x%x]\n", s32I2cAddr);
		g_aunSC501AI_2L_AddrInfo[ViPipe].s8I2cAddr = SC501AI_2L_I2C_ADDR;
	}
}
