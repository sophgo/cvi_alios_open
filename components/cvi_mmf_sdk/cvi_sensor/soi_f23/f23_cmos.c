...
#define F23_I2C_ADDR 0x40
#define F23_I2C_ADDR_IS_VALID(addr)	((addr) == F23_I2C_ADDR)
/****************************************************************************
 * global variables                                                            *
 ****************************************************************************/

ISP_SNS_STATE_S *g_pastF23[VI_MAX_PIPE_NUM] = {CVI_NULL};
SNS_COMBO_DEV_ATTR_S *g_pastF23ComboDevArray[VI_MAX_PIPE_NUM] = {CVI_NULL};
...
#define F23_SENSOR_GET_COMBO(dev, pstCtx)   (pstCtx = g_pastF23ComboDevArray[dev])
#define F23_SENSOR_SET_COMBO(dev, pstCtx)   (g_pastF23ComboDevArray[dev] = pstCtx)
...
ISP_SNS_COMMADDR_U g_aunF23_AddrInfo[VI_MAX_PIPE_NUM] = {
	[0] = { .s8I2cAddr = 0},
...
static CVI_S32 cmos_fps_set(VI_PIPE ViPipe, CVI_FLOAT f32Fps, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
	...
	if (WDR_MODE_2To1_LINE == pstSnsState->enWDRMode) {
		...
		if (g_astF23_State[ViPipe].u32Sexp_MAX < 2) {
			...
			CVI_TRACE_SNS(CVI_DBG_DEBUG, "VMAX %d, MAX_SEXP %d\n", u32VMAX, g_astF23_State[ViPipe].u32Sexp_MAX);
		}
	}
...
static CVI_S32 cmos_get_sns_regs_info(VI_PIPE ViPipe, ISP_SNS_SYNC_INFO_S *pstSnsSyncInfo)
{
	...
	if ((pstSnsState->bSyncInit == CVI_FALSE) || (pstSnsRegsInfo->bConfig == CVI_FALSE)) {
		...
		for (i = 0; i < pstCfg0->snsCfg.u32RegNum; i++) {
			...
			pstI2c_data[i].u8DevAddr = g_aunF23_AddrInfo[ViPipe].s8I2cAddr;
...
static CVI_S32 sensor_rx_attr(VI_PIPE ViPipe, SNS_COMBO_DEV_ATTR_S *pstRxAttr)
{
	...
	F23_SENSOR_GET_CTX(ViPipe, pstSnsState);
	F23_SENSOR_GET_COMBO(ViPipe, pstRxAttrSrc);
	...
	pstRxAttr->img_size.active_w = g_astF23_mode[pstSnsState->u8ImgMode].astImg[0].stWndRect.u32Width;
	pstRxAttr->img_size.active_h = g_astF23_mode[pstSnsState->u8ImgMode].astImg[0].stWndRect.u32Height;
	...
	pstRxAttr->img_size.max_width = g_astF23_mode[pstSnsState->u8ImgMode].astImg[0].stMaxSize.u32Width;
	pstRxAttr->img_size.max_height = g_astF23_mode[pstSnsState->u8ImgMode].astImg[0].stMaxSize.u32Height;
...
static CVI_S32 sensor_patch_rx_attr(VI_PIPE ViPipe, RX_INIT_ATTR_S *pstRxInitAttr)
{
	...
	if (!g_pastF23ComboDevArray[ViPipe]) {
		pstRxAttr = malloc(sizeof(SNS_COMBO_DEV_ATTR_S));
	} else {
		F23_SENSOR_GET_COMBO(ViPipe, pstRxAttr);
	}
	memcpy(pstRxAttr, &f23_rx_attr, sizeof(SNS_COMBO_DEV_ATTR_S));
	F23_SENSOR_SET_COMBO(ViPipe, pstRxAttr);
...
{
	if (g_pastF23ComboDevArray[ViPipe]) {
		free(g_pastF23ComboDevArray[ViPipe]);
		g_pastF23ComboDevArray[ViPipe] = CVI_NULL;
	}
	f23_i2c_exit(ViPipe);
}
...
{
	if (F23_I2C_ADDR_IS_VALID(s32I2cAddr))
		g_aunF23_AddrInfo[ViPipe].s8I2cAddr = s32I2cAddr;
	else {
		CVI_TRACE_SNS(CVI_DBG_ERR, "I2C addr input error ,please check [0x%x]\n", s32I2cAddr);
		g_aunF23_AddrInfo[ViPipe].s8I2cAddr = F23_I2C_ADDR;
	}
}
