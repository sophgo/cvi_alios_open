...
SNS_COMBO_DEV_ATTR_S *g_pastGc2053ComboDevArray[VI_MAX_PIPE_NUM] = {CVI_NULL};
...
#define GC2053_SENSOR_SET_COMBO(dev, pstCtx)   (g_pastGc2053ComboDevArray[dev] = pstCtx)
#define GC2053_SENSOR_GET_COMBO(dev, pstCtx)   (pstCtx = g_pastGc2053ComboDevArray[dev])
...
ISP_SNS_COMMADDR_U g_aunGc2053_AddrInfo[VI_MAX_PIPE_NUM] = {
	[0] = { .s8I2cAddr = 0},
...
static CVI_S32 cmos_get_sns_regs_info(VI_PIPE ViPipe, ISP_SNS_SYNC_INFO_S *pstSnsSyncInfo)
{
	...
	if ((pstSnsState->bSyncInit == CVI_FALSE) || (pstSnsRegsInfo->bConfig == CVI_FALSE)) {
		...
		for (i = 0; i < pstCfg0->snsCfg.u32RegNum; i++) {
			...
			pstI2c_data[i].u8DevAddr = g_aunGc2053_AddrInfo[ViPipe].s8I2cAddr;
...
static CVI_S32 sensor_rx_attr(VI_PIPE ViPipe, SNS_COMBO_DEV_ATTR_S *pstRxAttr)
{
	...
	GC2053_SENSOR_GET_CTX(ViPipe, pstSnsState);
	GC2053_SENSOR_GET_COMBO(ViPipe, pstRxAttrSrc);
	CMOS_CHECK_POINTER(pstSnsState);
	...
	pstRxAttr->img_size.start_x = g_stGc2053_mode.stImg.stWndRect.s32X;
	pstRxAttr->img_size.start_y = g_stGc2053_mode.stImg.stWndRect.s32Y;
	pstRxAttr->img_size.active_w = g_stGc2053_mode.stImg.stWndRect.u32Width;
	pstRxAttr->img_size.active_h = g_stGc2053_mode.stImg.stWndRect.u32Height;
	...
	pstRxAttr->img_size.max_width = g_stGc2053_mode.stImg.stMaxSize.u32Width;
	pstRxAttr->img_size.max_height = g_stGc2053_mode.stImg.stMaxSize.u32Height;
...
static CVI_S32 sensor_patch_rx_attr(VI_PIPE ViPipe, RX_INIT_ATTR_S *pstRxInitAttr)
{
	...
	if (!g_pastGc2053ComboDevArray[ViPipe]) {
		pstRxAttr = malloc(sizeof(SNS_COMBO_DEV_ATTR_S));
	} else {
		GC2053_SENSOR_GET_COMBO(ViPipe, pstRxAttr);
	}
	memcpy(pstRxAttr, &gc2053_rx_attr, sizeof(SNS_COMBO_DEV_ATTR_S));
	GC2053_SENSOR_SET_COMBO(ViPipe, pstRxAttr);
...
{
	if (g_pastGc2053ComboDevArray[ViPipe]) {
		free(g_pastGc2053ComboDevArray[ViPipe]);
		g_pastGc2053ComboDevArray[ViPipe] = CVI_NULL;
	}
...
static CVI_VOID sensor_patch_i2c_addr(VI_PIPE ViPipe, CVI_S32 s32I2cAddr)
{
	if (GC2053_I2C_ADDR_IS_VALID(s32I2cAddr))
		g_aunGc2053_AddrInfo[ViPipe].s8I2cAddr = s32I2cAddr;
	else {
		CVI_TRACE_SNS(CVI_DBG_ERR, "I2C addr input error ,please check [0x%x]\n", s32I2cAddr);
		g_aunGc2053_AddrInfo[ViPipe].s8I2cAddr = GC2053_I2C_ADDR_2;
	}
}
