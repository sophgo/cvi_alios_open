...
#define OS08A20_I2C_ADDR 0x36
#define OS08A20_I2C_ADDR_IS_VALID(addr) ((addr) == OS08A20_I2C_ADDR)
/****************************************************************************
 * global variables                                                            *
 ****************************************************************************/

ISP_SNS_STATE_S *g_pastOs08a20[VI_MAX_PIPE_NUM] = {CVI_NULL};
SNS_COMBO_DEV_ATTR_S *g_pastOs08a20ComboDevArray[VI_MAX_PIPE_NUM] = {CVI_NULL};
...
#define OS08A20_SENSOR_SET_COMBO(dev, pstCtx)   (g_pastOs08a20ComboDevArray[dev] = pstCtx)
#define OS08A20_SENSOR_GET_COMBO(dev, pstCtx)   (pstCtx = g_pastOs08a20ComboDevArray[dev])
...
ISP_SNS_COMMADDR_U g_aunOs08a20_AddrInfo[VI_MAX_PIPE_NUM] = {
	[0] = { .s8I2cAddr = 0},
...
static CVI_S32 cmos_set_wdr_mode(VI_PIPE ViPipe, CVI_U8 u8Mode)
{
	...
	case WDR_MODE_2To1_LINE:
		if (pstSnsState->u8ImgMode == OS08A20_MODE_2592X1944P30) {
			...
			CVI_TRACE_SNS(CVI_DBG_INFO, "WDR_MODE_2To1_LINE 1944p mode(60fps->30fps)\n");
		} else {
		}
...
static CVI_S32 cmos_get_sns_regs_info(VI_PIPE ViPipe, ISP_SNS_SYNC_INFO_S *pstSnsSyncInfo)
{
	...
	if ((pstSnsState->bSyncInit == CVI_FALSE) || (pstSnsRegsInfo->bConfig == CVI_FALSE)) {
		...
		for (i = 0; i < pstCfg0->snsCfg.u32RegNum; i++) {
			...
			pstI2c_data[i].u8DevAddr = g_aunOs08a20_AddrInfo[ViPipe].s8I2cAddr;
...
static CVI_S32 sensor_rx_attr(VI_PIPE ViPipe, SNS_COMBO_DEV_ATTR_S *pstRxAttr)
{
	...
	OS08A20_SENSOR_GET_CTX(ViPipe, pstSnsState);
	OS08A20_SENSOR_GET_COMBO(ViPipe, pstRxAttrSrc);
	...
	pstRxAttr->img_size.start_x = g_astOs08a20_mode[pstSnsState->u8ImgMode].astImg[0].stWndRect.s32X;
	pstRxAttr->img_size.start_y = g_astOs08a20_mode[pstSnsState->u8ImgMode].astImg[0].stWndRect.s32Y;
	pstRxAttr->img_size.active_w = g_astOs08a20_mode[pstSnsState->u8ImgMode].astImg[0].stWndRect.u32Width;
	pstRxAttr->img_size.active_h = g_astOs08a20_mode[pstSnsState->u8ImgMode].astImg[0].stWndRect.u32Height;
	...
	pstRxAttr->img_size.max_width = g_astOs08a20_mode[pstSnsState->u8ImgMode].astImg[0].stMaxSize.u32Width;
	pstRxAttr->img_size.max_height = g_astOs08a20_mode[pstSnsState->u8ImgMode].astImg[0].stMaxSize.u32Height;
...
static CVI_S32 sensor_patch_rx_attr(VI_PIPE ViPipe, RX_INIT_ATTR_S *pstRxInitAttr)
{
	...
	if (!g_pastOs08a20ComboDevArray[ViPipe]) {
		pstRxAttr = malloc(sizeof(SNS_COMBO_DEV_ATTR_S));
	} else {
		OS08A20_SENSOR_GET_COMBO(ViPipe, pstRxAttr);
	}
	memcpy(pstRxAttr, &os08a20_rx_attr, sizeof(SNS_COMBO_DEV_ATTR_S));
	OS08A20_SENSOR_SET_COMBO(ViPipe, pstRxAttr);
...
{
	if (g_pastOs08a20ComboDevArray[ViPipe]) {
		free(g_pastOs08a20ComboDevArray[ViPipe]);
		g_pastOs08a20ComboDevArray[ViPipe] = CVI_NULL;
	}
...
{
	if (OS08A20_I2C_ADDR_IS_VALID(s32I2cAddr))
		g_aunOs08a20_AddrInfo[ViPipe].s8I2cAddr = s32I2cAddr;
	else {
		CVI_TRACE_SNS(CVI_DBG_ERR, "I2C addr input error ,please check [0x%x]\n", s32I2cAddr);
		g_aunOs08a20_AddrInfo[ViPipe].s8I2cAddr = OS08A20_I2C_ADDR;
	}
}
