...
#define GC2145_I2C_ADDR_1 0x78
#define GC2145_I2C_ADDR_2 0x3c
...
SNS_COMBO_DEV_ATTR_S *g_pastGc2145ComboDevArray[VI_MAX_PIPE_NUM] = {CVI_NULL};
...
#define GC2145_SENSOR_SET_COMBO(dev, pstCtx)   (g_pastGc2145ComboDevArray[dev] = pstCtx)
#define GC2145_SENSOR_GET_COMBO(dev, pstCtx)   (pstCtx = g_pastGc2145ComboDevArray[dev])
...
ISP_SNS_COMMADDR_U g_aunGc2145_AddrInfo[VI_MAX_PIPE_NUM] = {
	[0] = { .s8I2cAddr = 0},
...
static CVI_S32 sensor_rx_attr(VI_PIPE ViPipe, SNS_COMBO_DEV_ATTR_S *pstRxAttr)
{
	...
	GC2145_SENSOR_GET_CTX(ViPipe, pstSnsState);
	GC2145_SENSOR_GET_COMBO(ViPipe, pstRxAttrSrc);
	CMOS_CHECK_POINTER(pstSnsState);
	...
	pstRxAttr->img_size.start_x = g_astGc2145_mode.stImg.stWndRect.s32X;
	pstRxAttr->img_size.start_y = g_astGc2145_mode.stImg.stWndRect.s32Y;
	pstRxAttr->img_size.active_w = g_astGc2145_mode.stImg.stWndRect.u32Width;
	pstRxAttr->img_size.active_h = g_astGc2145_mode.stImg.stWndRect.u32Height;
	...
	pstRxAttr->img_size.max_width = g_astGc2145_mode.stImg.stMaxSize.u32Width;
	pstRxAttr->img_size.max_height = g_astGc2145_mode.stImg.stMaxSize.u32Height;
...
static CVI_S32 sensor_patch_rx_attr(VI_PIPE ViPipe, RX_INIT_ATTR_S *pstRxInitAttr)
{
	...
	if (!g_pastGc2145ComboDevArray[ViPipe]) {
		pstRxAttr = malloc(sizeof(SNS_COMBO_DEV_ATTR_S));
	} else {
		GC2145_SENSOR_GET_COMBO(ViPipe, pstRxAttr);
	}
	memcpy(pstRxAttr, &gc2145_rx_attr, sizeof(SNS_COMBO_DEV_ATTR_S));
	GC2145_SENSOR_SET_COMBO(ViPipe, pstRxAttr);
	...
	if (pstRxAttr->input_mode == INPUT_MODE_BT601) {
		struct ttl_dev_attr_s *attr = &pstRxAttr->ttl_attr;
...
{
	if (g_pastGc2145ComboDevArray[ViPipe]) {
		free(g_pastGc2145ComboDevArray[ViPipe]);
		g_pastGc2145ComboDevArray[ViPipe] = CVI_NULL;
	}
...
static CVI_VOID sensor_patch_i2c_addr(VI_PIPE ViPipe, CVI_S32 s32I2cAddr)
{
	if (GC2145_I2C_ADDR_IS_VALID(s32I2cAddr))
		g_aunGc2145_AddrInfo[ViPipe].s8I2cAddr = s32I2cAddr;
	else {
		CVI_TRACE_SNS(CVI_DBG_ERR, "I2C addr input error ,please check [0x%x]\n", s32I2cAddr);
		g_aunGc2145_AddrInfo[ViPipe].s8I2cAddr = GC2145_I2C_ADDR_2;
	}
}
