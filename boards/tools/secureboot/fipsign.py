...
class SignedFIP(FIP):
    def __init__(self, root_priv_path, bl_priv_path, alios):
        super().__init__()
    ...
    def sign_alios_bl(self):
        logging.debug("sign_alios_bl:")

        sig_size = len(self.rsa_to_n(self.bl_priv))

        with open("boot", "rb") as fp:
            boot = fp.read()

        self.alios_bl["BOOT"].content = boot

        with open("prim", "rb") as fp:
            prim = fp.read()

        self.alios_bl["PRIM"].content = prim

        for name in ["BOOT", "PRIM"]:
            e = self.alios_bl[name]
            if not e.content:
                continue
        ...
        self.param2["ALIOS_BOOT_SIZE"].content = len(self.alios_bl["BOOT"].content)

    def make_alios_bl(self):
        logging.debug("make_alios_bl:")

        with open("boot_sign", "wb") as fp:
            fp.write(self.alios_bl["BOOT"].content)

        with open("prim_sign", "wb") as fp:
            fp.write(self.alios_bl["PRIM"].content)

class EncryptedFIP(SignedFIP):
    def __init__(self, root_priv_path, bl_priv_path, ldr_ek_path, bl_ek_path, alios):
        super().__init__(root_priv_path, bl_priv_path, alios)
    ...
    def encrypt(self):
        ...
        if self.alios:
            self.encrypt_alios_bl()

    def encrypt_alios_bl(self):
        logging.debug("encrypt alios bl:")
    ...
    def make_alios_bl(self):
        ...
        with open("boot_sign_enc", "wb") as fp:
            fp.write(self.alios_bl["BOOT"].content)

        with open("prim_sign_enc", "wb") as fp:
            fp.write(self.alios_bl["PRIM"].content)

def sign_fip(args):
    logging.debug("sign_fip:")

    fip = SignedFIP(args.root_priv, args.bl_priv, args.alios)
...
def encrypt_fip(args):
    ...
    fip = EncryptedFIP(args.root_priv, args.bl_priv, args.ldr_ek, args.bl_ek, args.alios)
    fip.read_fip(args.SRC_FIP)
...
def parse_args():
    ...
    for pr in [pr_sign, pr_encrypt]:
        ...
        pr.add_argument("--alios", type=bool)
